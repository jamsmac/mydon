...
// ── gap_state: журнал ПЕРЕХОДОВ пробела, а не список пробелов ──
//
// Реестр «Пробелы» по-прежнему вычисляется на чтении (R-K4) — эта таблица его
// НЕ заменяет и не кэширует. Она отвечает на единственный вопрос, на который
// вычисляемый список ответить не может: что изменилось со вчера. Без памяти о
// вчерашнем дне сигнала «стало известно» (Н-2) не существует физически —
// закрывшийся пробел просто исчезает между двумя чтениями, и никто этого не
// видит.
//
// Строка живёт ровно столько, сколько живёт ключ пробела; закрытый пробел не
// удаляется — иначе «что стало известно за сутки» опять неоткуда взять.
export const gapState = pgTable(
  "gap_state",
  {
    /** Устойчивый ключ строки реестра (`Gap.key`). Пробелы без ключа сюда не попадают. */
    key: text("key").primaryKey(),
    /** Тема и формулировка на момент последнего наблюдения — для подписи в сводке. */
    topic: text("topic").notNull(),
    missing: text("missing").notNull(),
    /** Ташкентские дни: когда пробел впервые увиден, когда видели последний раз. */
    firstSeenOn: date("first_seen_on").notNull(),
    lastSeenOn: date("last_seen_on").notNull(),
    /** День, когда пробел пропал из реестра. NULL — открыт сейчас. */
    closedOn: date("closed_on"),
    /**
     * День, когда закрытый пробел ВЕРНУЛСЯ. «Снова не считается» — не то же
     * самое, что «впервые не считается»: первое означает, что закрытие было
     * мнимым или данные откатились, и читать это надо иначе.
     */
    reopenedOn: date("reopened_on"),
    updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => [index("gap_state_closed_on_idx").on(t.closedOn), index("gap_state_first_seen_on_idx").on(t.firstSeenOn)],
);

// ── gap_reconcile_run: факт того, что сверка в этот день прошла ──
//
// Без него «сегодня ничего не закрылось» неотличимо от «сверка не запускалась»
// — а это разные новости: первая про спокойный день, вторая про простой.
// Вывести из самих пробелов нельзя: в день, когда реестр пуст, обеих новостей
// не видно вовсе.
export const gapReconcileRun = pgTable("gap_reconcile_run", {
  /** Ташкентский день. Один прогон в сутки — повтор перезаписывает счётчики. */
  day: date("day").primaryKey(),
  /** Сколько пробелов с устойчивым ключом увидели, сколько закрыли, завели и вернули. */
  gapsSeen: integer("gaps_seen").notNull(),
...
export const schema = {
  ...
  machinePlacement,
  // Переходы пробелов: «стало известно» (Н-2).
  gapState,
  gapReconcileRun,
};
