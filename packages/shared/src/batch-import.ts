...
/**
 * Окончания прилагательных, которые снимаются, чтобы существительное из имени
 * строки («лимон», «ягод») узнавалось в прилагательном из названия карточки
 * («Лимонный чай», «Ягодный чай»).
 *
 * ТОЛЬКО «ный» — и это не лень, а то же правило «не угадывать», ради которого
 * написан весь модуль. Проверено на живом реестре: одно это окончание даёт те
 * же 10 предложений и те же 84,3% денег, что и таблица из одиннадцати. Каждое
 * лишнее окончание — догадка без подтверждения, а некоторые прямо опасны:
 * «сухое» с окончанием «ое» даёт корень «сух», который начнёт цепляться к
 * посторонним словам. Появится карточка, которой нужно другое окончание —
 * добавим вместе с примером из данных и тестом.
 */
const ADJ_SUFFIXES = ["ный"] as const;
