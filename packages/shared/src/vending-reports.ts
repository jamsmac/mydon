...
export type AnalyticsWarningCode =
  ...
  | "history_capped"
  /**
   * Журнал прогонов НЕ ДОСТАЁТ до отчётной недели (R-FW-P5).
   *
   * СВОЙ код, а не текстовый вариант `health_unavailable`: тот значит «не
   * посчиталось» (секция деградировала), а здесь всё посчиталось верно —
   * данных за эту неделю не существует, потому что журнал начали вести позже.
...
export interface StockCountRow {
   ...
   * например `owner`;
   * `stock-import` — ВСЯ пометка переноса целиком, вида
   * «импорт истории mydon-stock · место: Холодильник»: правило —
   * `packages/shared/src/stock-history.ts` (`importNote(row.location_name)`),
   * запись — `packages/db/src/import-stock-history.ts`. МЕСТО — только хвост
   * этой строки, и достаёт его оттуда `placeFromImportNote`, а не разбор
   * пометки на месте.
...
export interface WeeklyHealth {
  ...
  week: string;
  /**
   * Прогонов, НАЧАТЫХ в [понедельник, следующий понедельник), — РОВНО СУММА
   * четырёх разрядов ниже.
   *
   * Считается сложением, а не длиной выборки, именно ради этого равенства:
   * строка письма печатает и итог, и разряды, и «прогонов 57 · успешных 54 ·
   * частичных 1 · отказов 1» владелец прочитал бы как ошибку отчёта. Разряды
   * покрывают весь союз статусов, поэтому невидимого остатка не бывает.
   */
  runs: number;
  /** Строго `status === "success"`. НЕ то же, что `lastDataAt` (см. его докблок). */
  success: number;
  /** Строго `status === "partial"`. Данные приехали не со всех автоматов. */
  partial: number;
  failed: number;
  /**
   * НЕЗАКРЫТЫХ прогонов недели (`status === "running"`).
   *
   * Отдельным разрядом, а не молчаливой разницей: для ПРОШЛОЙ недели
   * незакрытый прогон — это зависший сбор без `finished_at`, сигнал сам по
   * себе. Серию отказов он не рвёт и не продолжает (`worstFailedStreak` его
   * пропускает) — об итоге он не говорит ничего.
   */
  running: number;
  /** Самая длинная серия отказов ВНУТРИ недели. 0 — отказов не было. */
  worstFailedStreak: number;
  /**
   * Последний прогон недели, ДОНЁСШИЙ ДАННЫЕ — `success` ИЛИ `partial` (ISO).
   * `null` — таких прогонов в неделе не было ВОВСЕ (не «ноль часов»).
   *
   * НЕ `lastSuccessAt` И НЕ ПАРА К `success`. Правило «успех = донёс данные»
   * общее для репозитория (`sync-runs.ts`), но под именем со словом «успех»
   * оно спорило бы с разрядом `success`, который считает строго `success`:
   * неделя из одних `partial` дала бы «успешных 0» и тут же «последний успех
   * 23.08». Одно слово — одно значение, поэтому поле названо данными.
   *
   * Датируется ЗАВЕРШЕНИЕМ прогона, как и `health.lastSuccessAt`: «данные
  ...
  parityRed: number;
  /**
   * Неделя ЕЩЁ НЕ ЗАКОНЧИЛАСЬ на момент расчёта (`конец недели > now`).
   *
   * Понедельничное письмо про прошлую неделю всегда `false`; `true` бывает у
   * `?week=` на текущую неделю и у ручного вызова в середине недели. Без флага
   * «отказов 0» за неполные сутки недели читалось бы как итог всех семи.
   *
   * ЭТО НЕ РАЗРЯД `partial` (тот про статус ПРОГОНА, «данные приехали не со
   * всех автоматов»). Здесь неполна САМА НЕДЕЛЯ.
   */
  partialWeek: boolean;
  /**
   * Прогонов в неделе БОЛЬШЕ потолка чтения (`WEEK_RUNS_LIMIT`), числа
   * посчитаны по свежему хвосту окна.
   *
   * Молчаливое обрезание здесь хуже, чем в любой другой витрине: обрезанный
   * счёт выглядит как посчитанный, и «отказов 3» за неделю с зациклившимся
   * кроном увело бы владельца от аварии. Причина едет и в `warnings` письма.
   */
  capped: boolean;
  /**
   * Самый ранний прогон В ЖУРНАЛЕ — ташкентские сутки `YYYY-MM-DD`; `null`
   * значит, что журнал прогонов пуст ВОВСЕ (R-FW-P5).
   *
   * ЗАЧЕМ ПОЛЕ. `?week=` пускает 104 недели назад, а журнал `vending_sync_run`
   * на проде начинается 06.08.2026: всякая неделя до этой даты честно отдаёт
   * `runs: 0` и `lastDataAt: null` — и без даты начала журнала это читается как
   * «сбор не запускался», то есть как авария. Тот же довод, по которому рядом
   * заведён `ПАРИТЕТ_ВНЕ_ОКНА`: «не достали» и «не было» — два разных ответа.
   * Неделя, целиком лежащая раньше этой даты, получает предупреждение
   * `health_unavailable` в `warnings` письма.
   */
