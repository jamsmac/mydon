 ...
 * (ourvend-parity.service.ts). Недельная витрина (`сверено_пар`, `остатки_*`) и
 * ВЕРДИКТ ДНЯ (`день_*`) — в одном payload; серию судят только вторые.
 */
...
const красный = (date: string): ParityEventRow =>
  дн(date, { ok: false, расхождений: 2, день_ok: false, день_расхождений: 2 });
...
/** Форма П8b ДО правки P1b: обе половины есть, но полей дня ещё нет. */
const безДня = (date: string): ParityEventRow => ({
  occurredAt: new Date(`${date}T08:40:00+05:00`),
  payload: { ok: true, дней: 7, сверено_пар: 14, расхождений: 0, остатки_сверено: 68, остатки_расхождений: 0 },
});
...
describe("Серия зелёных дней паритета (R-P8b-1, R-P8b-2)", () => {
  ...
  it("ФОРМА БЕЗ ПОЛЕЙ ДНЯ НЕ ЗЕЛЁНАЯ: недельная витрина за вердикт дня не считается (P1b)", () => {
    // Событие с зелёной семидневкой, но без `день_*`: судить по нему — значит
    // растягивать один грязный день на неделю вердиктов, ровно то, ради чего
    // поля дня и заведены.
    const s = parityStreak([безДня("2026-08-25"), дн("2026-08-26")], 7, "2026-08-26");
    assert.deepEqual([s.greenDays, s.lastRed], [1, "2026-08-25"]);
  });

  it("вердикт дня СИЛЬНЕЕ недельной витрины в обе стороны", () => {
    // (а) неделя красная, день чистый — день зелёный: грязный день недельного
    // окна уже отстоял свои сутки и обязан выпасть из счёта.
    const чистыйДень = parityStreak(
      [дн("2026-08-26", { ok: false, расхождений: 4, остатки_расхождений: 2 })],
      7,
      "2026-08-26",
    );
    assert.equal(чистыйДень.greenDays, 1);
    // (б) неделя зелёная, день грязный — день красный.
    const грязныйДень = parityStreak([дн("2026-08-26", { день_ok: false, день_расхождений: 1 })], 7, "2026-08-26");
    assert.deepEqual([грязныйДень.greenDays, грязныйДень.lastRed], [0, "2026-08-26"]);
  });
  ...
  it("нулевая сверка остатков зелёной не считается", () => {
    ...
    const s = parityStreak([дн("2026-08-26", { день_остатков_сверено: 0 })], 7, "2026-08-26");
  ...
  it("расхождения по продажам красят день, даже если ok кто-то выставил в true", () => {
    ...
    const s = parityStreak([дн("2026-08-26", { день_расхождений: 3 })], 7, "2026-08-26");
    assert.deepEqual([s.greenDays, s.lastRed], [0, "2026-08-26"]);
    // Флаг дня в true при ненулевых расхождениях дня — верить надо числам.
    const о = parityStreak([дн("2026-08-26", { день_ok: true, день_расхождений: 1 })], 7, "2026-08-26");
  ...
  it("окно показа — 14 дней, свежие сверху, с числами обеих половин", () => {
    ...
    // Числа строки — ДНЕВНЫЕ (`день_*`), а не недельные: вердикт `ok` суточный,
    // и «сверено 14 пар» из недельного окна спорило бы с ним же.
    assert.deepEqual([s.days[0]!.salesChecked, s.days[0]!.stockChecked, s.days[0]!.ok], [2, 68, true]);
  ...
  it("записка дня доезжает до витрины: «расхождений 0» без объяснения читается как успех", () => {
    const s = parityStreak(
      [дн("2026-08-26", { день_остатков_сверено: 0, день_примечание: "снимков остатков нет" })],
      7,
      "2026-08-26",
    );
    assert.equal(s.days[0]!.note, "снимков остатков нет");
    assert.equal(s.days[0]!.ok, false);
    // У события старой формы записки дня нет — показываем недельную, иначе
    // строка была бы «расхождений 0» без объяснения.
    const старое = parityStreak([{ ...безДня("2026-08-26"), payload: { ...безДня("2026-08-26").payload, примечание: "снимков остатков OurVend за период нет" } }], 7, "2026-08-26");
    assert.equal(старое.days[0]!.note, "снимков остатков OurVend за период нет");
  });
