...
describe("Реестр закупок: строка заголовков не должна стать записью", () => {
  ...
  it("fileRow — номер строки в ФАЙЛЕ, как её видит человек в Excel", () => {
    // Именно так вызов и выглядит в жизни: результат parseXlsx передаётся как
    // есть, вместе со строкой заголовков внутри rows[0]. parseXlsx забирает в
    // columns только первую строку книги (титул), поэтому rows[0] — вторая
    // строка файла, и первая товарная строка это третья.
    const t = {
      columns: ["", "", "OOO VENDHUB", "", "", "", "", "", "", "", "", "", ""],
      rows: [
        ["", "", "Поставщик", "", "Наименование товара", "Ед. изм", "Кол-во", "Цена сум с НДС", "Стоимость сум с НДС", "Счет-фактура", "Оплата", "Дата оплаты", "Примечание"],
        ["1", "2025", "П", "1", "Товар", "кг", "1", "100", "100", "", "", "", ""],
      ],
    };
    const rows = parseRegisterRows(t, "2026-08-21", 2);
    assert.equal(rows.length, 1, "шапка отброшена");
    assert.equal(rows[0]!.fileRow, 3, "товарная строка — третья в файле");
    // Без указания номера считаем, что rows[0] и есть первая строка файла.
    assert.equal(parseRegisterRows(t, "2026-08-21")[0]!.fileRow, 2);
  });

  it("строка с количеством, но без стоимости — законная запись", () => {
    // Фильтр нетоварных строк обязан отбрасывать только те, где нет НИ
    // количества, НИ стоимости: заменить && на || значило бы молча потерять
    // строки, где владелец не проставил одну из двух колонок.
    const мк = (кол: string, стоим: string) => ({
      columns: [""],
      rows: [["1", "2025", "П", "1", "Товар", "кг", кол, "100", стоим, "", "", "", ""]],
    });
    assert.equal(parseRegisterRows(мк("5", ""), "2026-08-21").length, 1, "только количество");
    assert.equal(parseRegisterRows(мк("", "500"), "2026-08-21").length, 1, "только стоимость");
    assert.equal(parseRegisterRows(мк("", ""), "2026-08-21").length, 0, "ни того, ни другого");
  });
});
