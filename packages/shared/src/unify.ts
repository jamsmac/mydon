 ...
 *
 * Нужна ради дневного потока `sale` (синк OurVend через mydon-stock). Этот поток
 * уже свёрнут до дня — ни номера заказа, ни времени внутри дня в нём нет. Важно:
 * это ограничение ПОТОКА, а не самой платформы OurVend — в её кабинете есть
 * отчёт продаж по времени, и когда он ляжет в сырой слой отдельной выгрузкой,
 * он войдёт в союз построчно, как gjvending и vendinghub. Пока же дневной поток
 * сверяется по дню: свернуть союз до того же зерна — можно.
 *
 * Это НЕ добавка к выручке: OurVend показывает те же продажи третьим взглядом,
 * и сложить его с союзом значило бы задвоить.
 */
...
/**
 * Дневная сверка союза с дневным потоком OurVend (`sale`).
 *
 * Поток дневной, поэтому и сверка дневная, и так и написано. Это не приговор
 * платформе: её отчёт по времени, когда ляжет в сырой слой, войдёт в союз
 * построчно. Ни выручку союза, ни его число заказов эта дорожка не меняет —
 * она отвечает «сходится ли третий взгляд», а не слагаемое.
 */
...
/**
 * Сверить дневные корзины союза с дневными строками потока OurVend.
 *
 * Сравнение по выручке, округлённо: в дневном потоке нет заказов внутри дня, и
 * штуки с числом заказов союза сходятся не всегда, а деньги — величина, которую
 * обе стороны считают одинаково. Ни одно значение не объявлено верным: где
 * выручка разошлась, показаны обе.
 */
