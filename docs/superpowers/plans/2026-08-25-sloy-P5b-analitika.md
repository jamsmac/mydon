...
- Кеш и single-flight — дословно по `ShrinkageService`: `Map` по ключу `${отчёт}|${окно}|${tashkentDay(now)}`, TTL `REPORT_CACHE_MS = 5 * 60_000`, параллельные запросы одного ключа ждут ОДИН расчёт. `invalidate()` зовётся из `setSalePrice`/`bootstrapSalePrice` (эталон поменялся — кеш `price-gap` врёт).
...
**Files:** Create `apps/core/src/vending/weekly-digest.service.ts` (+test), `apps/core/src/ourvend/ourvend-health.service.ts` (+test); Modify `apps/core/src/vending/vending.controller.ts` (роут + DTO), `vending.module.ts`, `apps/core/src/ourvend/ourvend.controller.ts` (`@Get("health")`), `ourvend.module.ts`, `apps/core/src/vending/vending.service.ts` (`finishSyncRun`), `apps/core/src/rules/rules.ts` (+`rules.test.ts`), `packages/shared/src/vending-reports.ts` (+test: типы `WeeklyDigest`, `OurvendHealth`), `tools/smoke-core.mjs`.
...
export interface WeeklyDigest {
  ...
  worstProducts: MarginProduct[];    // худшие-3 по проценту
...
3. **Прод, только чтение** — сверить с `inventory-prod.md` §9:
   - `GET /vending/margin?days=30` → 2 автомата, 1047 шт, выручка **8 974 000**, закуп **6 500 879**, маржа **2 473 121 (27.6 %)**; Olma **1 621 385 (27.6 %)**, American Hospital **851 736 (27.5 %)**; 34 SKU, отрицательной маржи 0, `unknownUnits = 0`; в `excluded` — строка SKLAD 4S от 09.07 (1 шт / 12 000).
   - `GET /vending/dead-stock?days=21` → склад **0 позиций**; автоматы **5 строк, 28 шт, 290 500 сум** (Kinder Bueno 11+2, Cheers 5+5, TUC 5). Если вылезли 129 строк и ~45 млн — сломан фильтр `in_service` (R-P5b-1), откатывать.
   - `GET /vending/price-changes?days=60` → по витрине **ровно одна находка**: LaimonFresh Lime 330ml 15000→12000 (−20.0 %, 08.07); закупочная лента пуста (событий по цене на проде 0).
   - `GET /vending/price-gap?days=14` → **0 строк**, `noReference` — все **34 SKU** («эталон не задан»).
   - `GET /vending/weekly-digest?week=2026-34` → 248 шт, 2 157 000 сум, маржа 607 595; заливок 3 (183 ед), записано мастером 0; приходов 0; инвентаризаций 0; отказов синка 0 (серия началась 24.08, это уже неделя 35).
   - `GET /ourvend/health` → `failedStreak` ≥ 12 (или 0, если хот-фикс синка уже выкачен), лаг слотов ~0.8 ч, продаж ~10.7 ч, `product_sale` ~36.8 ч (красный), паритет 14/14 без расхождений.
...
| R-P5b-4 мёртвый сток 21 день | T1 `deadStock`, T3 сбор `moved` из продаж/заливок/накладных | T1 «5 строк, 28 шт, 290 500», «флаг по паре (автомат, товар)», T3 «заливка снимает флаг» |
