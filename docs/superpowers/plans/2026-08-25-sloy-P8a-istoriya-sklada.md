...
- Вставки: `vending_refill` — `onConflictDoNothing({ target: vendingRefill.clientKey })`; `vending_stock_count` — `onConflictDoNothing({ target: [vendingStockCount.source, vendingStockCount.extId], where: sql`${vendingStockCount.extId} is not null` })` (у drizzle 0.45 предикат частичного индекса называется `where`, не `targetWhere` — тот существует только у `onConflictDoUpdate` и не собирается; см. аддендум спеки); `purchase` (только `missing`) — `onConflictDoNothing({ target: [purchase.source, purchase.extId] })`. `written` считается по длине `returning()`, а не по длине входа: иначе повторный прогон отчитался бы «записано 107», записав ноль.
