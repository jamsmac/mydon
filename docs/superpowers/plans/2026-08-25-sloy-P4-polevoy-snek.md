...
- Никаких изменений на проде из задач плана; «мёртвый автомат» = все валидные слоты с товаром `quantity >= capacity` при ≥ 10 таких слотов (R-P4-4).
...
/** R-P4-4: все валидные слоты с товаром полны и их ≥ DEAD_MIN_SLOTS → данных нет, автомат мёртв. */
...
```
Семантика: количества по товару = Σ `min(quantity, capacity)` по валидным слотам с этим товаром; товар считается в дне, если есть и в start, и в end (иначе день по товару пропускается); `lossValue = Σ max(loss,0) × price` (нет цены → `noPrice`, в деньги 0); `alert = lossValue ≥ threshold`.
...
Паритет остатков: `OurvendParityService.parity(days)` дополнительно `stock: { days, checked, ok, mismatches[{dt, serial, product, own, stock}] }` — сравнение `ourvend_stock_snapshot` ↔ `machine_stock` по `(dt, serial, product)` за `days` (только автоматы, у которых есть обе стороны); вердикт `ok` = обе части (продажи и остатки) без расхождений; `daily()` пишет в сводку обе. Тесты со стабами.
