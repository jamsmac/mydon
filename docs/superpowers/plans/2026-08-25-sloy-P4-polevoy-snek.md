...
Полный прогон: `pnpm -s lint && pnpm -s typecheck && pnpm -s build && pnpm -s test`; smoke-core на scratch-БД (createdb → migrate → seed.js → seed-vending.js → backfill-product-ids.js → smoke → dropdb).
