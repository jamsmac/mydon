...
Волна фиксов после финального ревью + adversarial (`адверсариал-прод-данные`,
`-конвенции`, `-ух`) уточнила четыре рулинга. §3.2, §7 и план (Global
Constraints, Task 3/4) выше НЕ переписаны построчно ради истории решения —
актуальная формулировка здесь.

**R-P4-4 уточнён (было — противоположное правило).** `deadMachine` НЕ «все
валидные слоты с товаром полны (`quantity >= capacity`)» — это старое правило
ложно срабатывало на только что заправленном живом автомате (43/43 слота
5/5 сразу после заливки выглядят так же, как заглушка). Действующее правило:
слотов с товаром ≥ `DEAD_MIN_SLOTS` (10), И ни одна из их ёмкостей НЕ валидна
(вне диапазона `0..MAX_CAPACITY`) — источник отдаёт мусор вместо данных, а не
просто «склад заправлен под завязку». На проде (инвентаризация 25.08) эта
ветка не срабатывает НИ РАЗУ: склад-заглушки SKLAD 5S/6S отдают
`quantity=capacity=199`, но `product_name` у них пуст → `hasProduct=false` →
`withProduct=0` → причина `no_slots`, не `dead`; SKLAD 4S даёт 43 имени и 7
валидных ёмкостей из 43 → `uncalibrated`. `dead` остаётся в коде как защитный
резерв на источник, который однажды начнёт отдавать имя товара при
невалидной ёмкости — не мёртвый код, но и не наблюдавшийся путь.

**R-P4-3 уточнён — неттинг внутри товара за период (R-FW-1, было —
посуточный `Σ max(loss,0)`).** Прод кладёт продажи OurVend в `sale.dt` со
сдвигом ±1 день относительно снимков остатков: у одного товара день −3 и
день +3 идут подряд. Посуточная сумма положительных потерь считала каждую
такую пару дважды — «потеря 3» в один день и «излишек 3» в соседний, вместо
взаимного гашения. Формула: `net = Σ(start − end − sales)` по всем
посчитанным дням товара за период; `lossUnits = max(0, net)`,
`surplusUnits = max(0, −net)`, `lossValue = lossUnits × price`. Между
товарами по-прежнему ничего не гасится. `SHRINK_ALERT_UZS` не менялся (30000).

**Паритет остатков: `checked = 0` ⇒ `stock.ok = false` с note (R-FW-2, было —
`checked=0` молча давал `ok=true`).** Пустой `ourvend_stock_snapshot` за
период — это «нечего сверять», а не «сверено и сошлось»: старое поведение
дало бы зелёный гейт «7 дней подряд» на проде, где вообще ни одна строка не
сравнивалась (прод read-only 25.08: `dt < current_date` выбрасывал
единственный день снимков целиком). Исправлено: `stock.checked = 0` ⇒
`stock.ok = false`, `note: "снимков остатков OurVend за период нет — сверять
не по чему"`. Складские/не-в-строю автоматы исключаются из сверки явно тем же
реестром (`notInService`/`skipReasonOf`), а не пересечением множеств сторон.

**`vending.refill_detected` эмитится по несопоставленным окнам старше 3 ч
(R-FW-9, было — в момент детекта).** Сценарий на проде штатный: детектор
бежит после каждого сбора слотов (раз в 3 ч), техник дописывает позицию в
боте уже ПОСЛЕ прогона — событие «без записи» уходило в брифинг раньше, чем
человек успевал отчитаться, и ack брифинга необратим. Событие эмитится
