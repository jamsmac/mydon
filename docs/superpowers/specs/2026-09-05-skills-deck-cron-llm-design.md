...
- Дубли согласований `chief-of-staff` починены (22ad1fb, `signatureFacts`). Для cron llm этого НЕ хватило —
  реальное правило дедупа см. §13 (строки F2): сигнатура считается по входу задачи и собранному промпту,
  а описание задачи по расписанию не несёт планового времени.
...
### Раунд правок после adversarial-ревью ветки (05.09.2026)

Ревью всей ветки нашло семь подтверждённых расхождений; ниже — как каждое закрыто.

| Найдено | Как исправлено | Почему |
|---|---|---|
| **F1.** Явный, но не реализованный навык молча подменялся угадыванием: worker принимал `agentSkill` только при `agent.skills.includes && hasSkill`, иначе шёл в `matchSkill` (draft-quote → qualify-lead, draft-reminder T3 → watch-receivables T1), а `task.agent_skill` и deck показывали нажатое имя | Три слоя. Worker: `resolveTaskSkill` возвращает `{skill, reason?}` и у явного навыка НЕ откатывается на подбор — не реализован, значит durable block с текстом «Навык «x» задан явно, но не реализован — угадывать не буду. Реализованные навыки: …». Core `runSkill`: 409 при `executor: "code" && hasCode === false`. Deck: `implemented = executor === "llm" \|\| hasCode`, кнопка заблокирована, пилюля «не реализован». Смоук: запуск нереализованной строки → 409 | Решение Р-6: «угадывание, поправляющее явное указание, — худший из отказов, потому что он молчаливый». В каталоге 30 строк, реализованы 10 — молчаливая подмена была правилом, а не краем |
| **F2.** Дедуп cron llm не срабатывал никогда: `agentScheduleIdentity` клал в описание `Плановое время UTC: <ISO>` → уникальный `inputHash` на каждый тик → `no_change` мёртв, каждое срабатывание открывало новое согласование | (а) Описание задачи по расписанию — `Системный запуск навыка <навык>.\nCron: <cron>`; время срабатывания живёт в `due`. (б) В `signatureFacts` добавлен `promptHash` (sha256 собранного `system`: устав, роль, тело навыка, KB) | Дедуп обязан ловить «тот же повод». Один `inputHash` этого не даёт в обе стороны: волатильное время ломало подавление, а правка KB/паспорта иначе прошла бы незамеченной. Правило теперь: **одинаковый вход и промпт → `no_change`; новое согласование только при смене входа, KB или паспорта** |
| **F3.** Снимок агентов у worker отстаёт до 10 минут от карточки в Core — свежевписанный владельцем навык отменялся устаревшим `agent.skills` | Проверка членства из worker снята (см. F1); членство проверяет Core в момент создания задачи (R-SD-6) | Источник истины о навыках агента — Core, а не 10-минутный снимок. Проверять дважды по разным по свежести данным значит иногда отменять решение владельца |
| **F4.** `modelEffort: "minimal"` принимался DTO Core и мэппером агентов, но отвергался provider-валидатором (`OPENAI_GPT_56_REASONING_EFFORTS`) → задача крутилась в 60-секундных повторах | `minimal` убран из `MODEL_EFFORTS` Core, из набора мэппера claim и из `EFFORTS` `skill-loader` (там теперь замечание паспорта). Тип `ModelReasoningEffort` в `model-gateway` не тронут | Список принимаемого обязан совпадать со списком исполняемого. Отказ при вводе дешевле бесконечного повтора |
| **F5.** llm-навык вставал на cron даже при выключенном/не-metered маршруте: задача на каждый тик → `route_unavailable` → повтор каждые 60 секунд навсегда | `desiredJobs` получает `(s) => hasCodeSkill(s) \|\| (isLlmSkill(s) && llmCronAdmitted(modelGatewayFromEnv()))`; `llmCronAdmitted` — чистая функция в `schedule.ts`. Отказанные так навыки попадают в лог «Навыки без реализации (не планируются)» с причиной «LLM-маршрут выключен/не metered» | Legacy-путь fail-closed ещё до планирования; llm обязан вести себя так же. Reconcile идёт раз в 10 минут — включат маршрут, и навык встанет сам |
| **F6.** Частичный индекс `task_agent_skill_idx` был только в рукописном SQL — дрейф схемы и снапшота (следующая генерация создала бы его заново) | Индекс объявлен в `schema.ts` (`.on(ownerRef, agentSkill, createdAt.desc()).where(agent_skill is not null)`), миграция 0087 **перегенерирована** drizzle-kit под тем же тегом с восстановленным русским заголовком; рукописного SQL не осталось | Конвенция репо (`task_agent_schedule_open_queue_idx` рядом). 0087 ещё не на проде — перегенерация безопасна |
| **F7.** Класс `.pill.warn` использовался в разметке deck, но правила не имел — «исполнится код» выглядел обычной серой меткой | Правило добавлено рядом с `.pill.ok/.bad`: `--hot` / `--hot-line` / `--hot-soft` (токены есть во всех трёх ветках темы) | Метка расхождения файла с реальностью обязана читаться как предупреждение, иначе она не предупреждает |

Тесты as-built после раунда (все зелёные): core **1832**, agents **499**, cc **403**, shared **854**, bot **828**, db **170**.
