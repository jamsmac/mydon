...
8. **`productIndex` (построитель индекса алиас→канон) живёт в
   `packages/shared/src/stock-history.ts`, а не в `packages/db`.**
   `backfill-product-ids.ts` резолвил `product_id` по имени своей копией
   правила нормализации/алиасов; план не заводил для неё общего места.
   Реализация (Task 3 fix, `9ebf999`) вынесла построение индекса в
   `@mydon/shared` и превратила `backfill-product-ids.ts` в тонкую обёртку
   над ним — тот же канон, что использует разовый импорт, вместо второй,
   потенциально расходящейся реализации резолва имени.
...
9. **Новый файл `apps/core/src/ourvend/sync-runs.ts`** (`lastSuccessRunAt()`,
...
10. **Окно `GET /vending/stock-counts?days=` — «последние N суток ВКЛЮЧАЯ
...
## Уточнения после adversarial (волна фиксов, 26.08)

Прод-ревью (`adversarial-prod-data.md`) и ревью безопасности
(`adversarial-security.md`) прогнали Task 3/4/5 на живых данных прода
`46.62.144.36` и на модели угроз «разовый скрипт на боевых доступах». Ниже —
рулинги волны фиксов (R-FW-P1…P4, R-FW-S1…S4), которые уточняют, но не
отменяют R-P8a-1…9: числа выкатки (342/107/460/455/603) и границы среза не
меняются, меняется точность резолва имён, читаемость истории по трём
локациям, окно чтения и семантика «успеха» для сторожа.

- **R-FW-P1 (уточняет R-P8a-7).** Резолв имени донора шёл только по
  `products.name`, хотя `products.ourvend_name` — точный алиас каталога
  OurVend/mydon (`inventory-donor.md` §1). Без этого 243 из 567 строк (43%)
  уезжали с `product_id = NULL` при живом мосте: 13 имён/228 строк
  разрешаются точным алиасом (`Coca Cola CAN 0.25`→`Coca-Cola Classic CAN
  0,25`, Pepsi CAN 0.25, Fuse Tea Mango, Laimon/Moxito Fresh Lime/Klubnika,
  Barni, Borjomi, ChocoPie, Kinder Bueno, Nesquick, TUC, Coca Cola Zero).
  Правка: оба донорских SELECT (заливы, инвентаризации) отдают
  `p.ourvend_name`; резолв — `canonicalProductName(name) ?? canonicalProductName(ourvendName)`,
  сопоставление остаётся ТОЧНЫМ, нечёткости не добавляется. После правки
  без карточки — **11 имён / 15 строк** (было 24 имени / 243 строки).
- **R-FW-P2 (уточняет R-P8a-3).** «Склад» донора — не одна точка, а три
  локации (`1 Склад (основной)` 423 строки, `2 Холодильник` 20, `3 Oq
  apparat (склад)` 17). Без признака места две строки с одинаковыми `dt` и
  товаром из разных локаций читались бы как случайный двойной ввод (4 пары
  дат/товаров 05.07.2026 лежат в двух местах разным qty). Правка: SELECT
  джойнит `locations`, имя места идёт в `note`; строки остаются раздельными
  (разные `ext_id`, дедупа по естественному ключу по-прежнему нет).
- **R-FW-P3 (уточняет R-P8a-3 / интерфейс `GET /vending/stock-counts`).**
  Потолок окна `days` был 365 (заимствован у соседних отчётов), а
  начальный остаток донора — 2025-08-17, 26 строк старше 365 суток от
  текущей даты — не читались НИКАКИМ клиентом никогда. Потолок поднят до
  **730** суток (дефолт остаётся 90); DTO и тест на границу 730 обязательны.
- **R-FW-P4 (уточняет R-P8a-6).** Сторож считал «успехом» только
  `status='success'`, а `failedStreak` — только `failed`; статус `partial`
  (данные приехали частично, коллектор жив) не продлевал `lastSuccessAt`,
  и два `partial` подряд выглядели как 6 часов простоя, хотя сбор работал.
  Правка: `partial` считается успехом для `lastSuccessAt`/`staleHours`
  ровно так же, как `success`; `failedStreak` по-прежнему считает только
  `failed` — эти два счётчика ловят разное (застой vs. серия явных отказов),
  и их нельзя путать.
- **R-FW-S1 (безопасность, смоук).** Застава `tools/smoke-import.mjs`
  печатала `DATABASE_URL` целиком (с паролем) в сообщениях об отказе и в
  выводе дочернего процесса. Правка: печатается только `new URL(url).host`
  (или `***`), полная строка подключения — никогда, ни в заставе, ни в
  тексте ошибок.
- **R-FW-S2 (безопасность, маппинг донора).** Ни величина чисел, ни
  управляющие символы в имени не проверялись — одна враждебная строка
  донора (переполнение `integer`/`numeric(12,2)`, `U+0000` после
  `decodeHtml`) роняла пачку в 500 строк, и на повторном прогоне падение
  повторялось бы. Правка: рядом с существующей заставой `fractional_qty` —
  застава на диапазон колонки (`qty` > `2 147 483 647` для заливов, ≥ `1e10`
  для пересчётов), нечисло/бесконечность и `\p{Cc}` (включая `U+0000`) в
  имени после `decodeHtml` → новая причина отказа строки (`out_of_range`,
  `control_chars`), а не падение импорта; в jsonb-событие имена уходят
  только после той же чистки.
- **R-FW-S3 (безопасность, сверка закупок).** У сверки закупок заставы
  годности не было вовсе (в отличие от заливов/пересчётов): `qty` молча
  писался нулём вместо отказа, `dt`-мусор уходил в `date NOT NULL`. Правка:
  `qty` — число > 0 (иначе `differing`/пропуск с причиной), `dt` — валидная
  дата, `unit` ≤ 64 символов; негодная `missing`-строка уходит в отчёт
  причин, а не в зеркало.
- **R-FW-S4 (безопасность, смоук на scratch).** Заставы смоука смотрели
  только на hostname и на отсутствие строк импорта — SSH-туннель на
  `localhost:5432`, указывающий на боевую БД, проходил обе заставы, и до
  первого `--apply` строк импорта там ноль. Правка: смоук требует явный
  `SMOKE_SCRATCH=1` в окружении (CI его выставляет) ЛИБО имя базы,
  начинающееся с `p8asmoke_`/содержащее `smoke`; боевой IP и имя
  пользователя убраны из фикстуры смоука — вместо них адреса из
  документационного диапазона (RFC 5737-style, не боевые/не реальные хосты)
  и подставное имя пользователя.
- **R-FW-S7 (принято, без правки кода).** Две узкие гонки — проверка
  `person` до транзакции пересчёта и дедуп сторожа `select→insert` без
  уникального индекса — приняты как риск при одной живой реплике Core и
  таблице `person` без soft-delete; решение зафиксировано комментарием в
  коде, поведение не меняется.
- **Бэклог волны фиксов (не в этом срезе):** ретенция `vending_stock_count`
  (растёт без чистки — читатели прикрыты потолком 2000, сама таблица нет);
  `GET /vending/stock-counts` открыт без токена (тот же класс риска, что и
  прежний бэклог «read-token для денежных GET», П5b); уникальный индекс для
  дедупа сторожа застоя — если Core когда-нибудь станет многорепличным.
