...
Цепочка резолва не меняется: алиас → `ourvend_name` → имя, ровно тот же
`productIndex` из `@mydon/shared`. Добавляются две цели: `vending_refill`
...
- **S2.** `POST /system/retention/run` (`ServiceTokenGuard`, `@Throttle`
  burst), тело `{ dryRun?: boolean }` — ручной прогон ретенции: единственный
  сырой SQL среза (`date < голые сутки`) впервые исполняется живым Postgres
  не по расписанию, а по вызову; шаг `tools/smoke-core.mjs` на scratch
  проверяет это на вставленной строке с `dt` на 800 суток в прошлом.
  Документация — `docs/DEPLOY.md`, раздел «Катовер учёта OurVend (П8b)».
- **S3.** Сторож коллизии в `resolveProductIds`: если сырое имя резолвится
  через алиас, но само по себе точно совпадает с каноническим именем ДРУГОЙ
  карточки — конфликт, строка печатается как «конфликт» и не привязывается.
  `--dry-run` печатает карту `raw → каноническое имя` с пометкой источника
  (карточка напрямую / алиас); `ourvend_name` в этой карте не участвует — это
  поле донора mydon-stock, доступное только импорту истории, не бэкфиллу
  (подтверждено прод-ревью, `adversarial-prod-data.md`).

### UX-1…6 (панель и бот — B)

Тексты — цитаты рулингов `adversarial-ux.md`; точная форма — за `apps/cc`/
`apps/bot`, см. `fix-wave-B-report.md` для финальной сверки.

- **UX-1.** Заголовок группы импорта без места → «место не указано» (без
  технических строк вроде «импорт истории mydon-stock»).
- **UX-2.** Свои строки: подпись «кто считал» показывается, только если
  `note` ≠ `'owner'`/не пуст; для `'owner'` заголовок группы — «Владелец»
  (перевод системного литерала), иначе — «инвентаризация MYDON».
- **UX-3.** «с {since}» печатается только когда `!history_capped`; при
  обрезке — «показаны последние 2000 записей, с {since}», где `{since}` —
  дата самой старой из ПОКАЗАННЫХ строк.
- **UX-4.** Текст без ключа настройки: «порог обнаружения — в настройках
  («Система» → порог заливки)» вместо голого `REFILL_DETECT_MIN_UNITS`.
- **UX-5.** `health_unavailable`/`ПАРИТЕТ_ВНЕ_ОКНА`: «сверка за эту неделю
  недоступна — напишите боту «сверка»» вместо несуществующего для владельца
  адреса `/ourvend/health`.
- **UX-6.** Бот: убрать/переименовать дублирующую строку «Прогоны (N)» — одна
  строка «За неделю: прогонов N (успешных/частичных/отказов), сейчас: застой
  …»; минорные тексты — убрать префикс «Журнал заливок ·» в лиде, пустое
  состояние «Истории склада» с окном и подсказкой «расширьте до 730»,
...
- **P1.** Резолв бэкфилла складывает разделитель дробной части (`,` → `.`)
  в нормализацию имени товара — на живом каталоге закрывает 4 из 11
  неразобранных имён `vending_stock_count` без единого действия владельца
  ...
  0.25` — уже случившийся на проде пример ровно этой ошибки).
- **P2.** `ORDER BY` истории склада получает тай-брейкер (`id`/`ext_id`):
  на проде 46 пар (день, товар, место) содержат по ДВЕ строки (у 41 из них
  числа разные), и без тай-брейкера порядок между ними отдавал сам Postgres,
  не гарантированно одинаковый между двумя чтениями.
...
- **P4.** «Сколько раз считали склад» в недельном письме (`stocktakes`)
  переключено на `vending_stock_count` по `dt` — прежний счёт по
  `vending_stock` (перезаписной таблице ТЕКУЩЕГО остатка) занижал прошлые
  недели до нуля и «плыл» задним числом на каждом новом пересчёте.
- **P5.** `WeeklyHealth` получает поле с датой начала журнала прогонов
  (`vending_sync_run` заведён 06.08.2026) и предупреждение для недель раньше
  этой даты — без него такие недели молча получали `runs: 0` без единого
  слова о причине. Финальное имя поля и текст предупреждения — код
  `apps/core/src/vending/weekly-digest.service.ts` и
  `fix-wave-A-report.md`; `docs/DATA_SOURCES.md` заведён рабочим именем
  `journalSince` до сверки с итоговым кодом.
