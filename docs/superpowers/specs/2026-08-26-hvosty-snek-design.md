...
- **S2.** `POST /system/retention/run` (`RetentionController`, путь
  ...
  **Найдено этим же маршрутом (не в задании, но обязательный побочный
  фикс):** `Date` в сыром параметре шаблона `sql` уходит в postgres.js без
  типа колонки и падает ДО сервера («Received an instance of Date») — с
  выкатки П8b четыре из пяти целей ретенции (`slot_snapshot`, `product_sale`,
  `machine_sale`, `vending_sync_run`) не чистились НИ РАЗУ, юнит-стенд этого
  не видел принципиально (рендерит `JSON.stringify(params)`, а не исполняет
  запрос). Исправлено (`RetentionService.граница()` — граница всегда строка)
  коммитом `3fc4e8f`, ДО первого воскресенья после мержа; `docs/DEPLOY.md`
  и `docs/PLAN_STOCK_ABSORPTION.md` предупреждают, что первый настоящий
  прогон почти наверняка уйдёт в `capped: true` на нескольких целях —
  штатно, не авария.
...
буквально с диффом. UX-5 на финальном HEAD (`apps/core/src/vending/weekly-digest.service.ts`,
коммиты `9f2e29f`/`3fc4e8f`) НЕ реализована по рекомендованному тексту:
адреса `/ourvend/health`/`/ourvend/parity/streak` остались как были. B явно
зафиксировал это в своём отчёте («UX-5 не мой… адрес для владельца остаются
в текстах Core»), А в свой список пунктов задания UX-5 не включил — задача
осталась за пределами волны фиксов. Ниже — фактический текст на HEAD, не
рекомендация ревью.
...
- **UX-5 (НЕ закрыт — оставлен как есть, не рекомендацией ревью).** Ровно
  цитаты `git show HEAD:apps/core/src/vending/weekly-digest.service.ts`:
  «сейчас» здоровье — `Здоровье сбора не посчиталось (${причина}) — деньги
  недели в письме честные, а про сбор смотри /ourvend/health.»; за неделю —
  ...
  паритет за неделю не посчитался — «Дни паритета за неделю ${неделя.key} не
  посчитались (${паритет.failure}) — прогоны сбора в письме настоящие, а
  сверку смотри в /ourvend/parity/streak.»; паритет вне окна серии
  (`ПАРИТЕТ_ВНЕ_ОКНА`) — «Дни паритета за эту неделю вне окна счёта серии
  (14 дней) — смотри /ourvend/parity/streak»; обрезка журнала прогонов
  (`ПРОГОНЫ_ОБРЕЗАНЫ`) — «Прогонов за неделю больше потолка чтения (200) —
  числа блока посчитаны по самым свежим прогонам окна, весь журнал в
  /ourvend/health.». Ни один из этих пяти адресов не открывается владельцем
  напрямую (нет ни бот-команды, ни листа панели с таким путём) — находка
  M5 адверсариал-UX остаётся в силе как задокументированный остаток.
