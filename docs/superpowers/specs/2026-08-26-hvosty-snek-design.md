...
(`apps/cc`); UX-6 (бот) — коммитом `7ffdde9` (`apps/bot`) — обе сверены
буквально с диффом. UX-5 (`health_unavailable`/`ПАРИТЕТ_ВНЕ_ОКНА` в
`apps/core/src/vending/weekly-digest.service.ts`) на момент этой сверки ещё
в работе (файл в рабочем дереве изменён, но не закоммичен) — не проверена по
коду, только по тексту рулинга.
...
- **P5.** `WeeklyHealth.journalSince: string | null` — ташкентские сутки
  ...
  `docs/DATA_SOURCES.md`. Бот печатает («хвосты, `7ffdde9`»): «За неделю
  прогонов нет: журнал прогонов начинается с {ДД.ММ.ГГГГ} — за эту неделю
  данных нет» — вместо ложного «сбор не запускался». Проверка на основании
  недельного письма Core (`weekly-digest.service.ts`, предупреждение
  `health_unavailable` про паритет) — отдельно, ещё в работе.
