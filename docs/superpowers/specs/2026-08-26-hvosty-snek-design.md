...
- **S3.** `productIndex.explain` (`packages/shared/src/stock-history.ts`) —
  ...
  `ourvend_name` в этой карте не участвует — это поле донора mydon-stock,
  доступное только импорту истории, не бэкфиллу (подтверждено прод-ревью,
  `adversarial-prod-data.md`). Реализовано и слито коммитом `aa6ff93`.
...
(`apps/cc`) — сверены буквально с диффом. UX-5/UX-6 (бот) на момент этой
сверки ещё в работе (`apps/bot/**` в рабочем дереве не закоммичен) — не
проверены по коду, только по тексту рулинга.
...
- **UX-5.** `health_unavailable`/`ПАРИТЕТ_ВНЕ_ОКНА`: «сверка за эту неделю
  ...
  адреса `/ourvend/health`. В работе на момент сверки — не подтверждено
  кодом.
- **UX-6.** Бот: убрать/переименовать дублирующую строку «Прогоны (N)» — одна
  ...
  …»; минорные тексты — убрать префикс «Журнал заливок ·» в лиде (✅
  ...
  `72b1245`, `RefillEventsView` теперь читает `{ rows, capped }`). Строка
  «Прогоны (N)» бота — в работе, не подтверждена.
...
- **FR-M1.** `VendingPurchase`/`VendingPurchaseItem` в
  ...
  `apps/cc` слито коммитом `72b1245`; `apps/bot` — в работе на момент сверки.
...
- **P5.** `WeeklyHealth.journalSince: string | null` — ташкентские сутки
  ...
  вовсе); без него недели раньше 06.08.2026 молча получали `runs: 0`. Имя
  поля подтверждено кодом (`packages/shared/src/vending-reports.ts`, коммит
  `aa6ff93`) — совпадает с тем, что уже документировано в
  `docs/DATA_SOURCES.md`. Печать предупреждения в письме бота (`apps/bot`) на
  момент сверки ещё в работе.
