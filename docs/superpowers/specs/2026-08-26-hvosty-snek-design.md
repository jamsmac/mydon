...
validate: atLeast(365, "нужно не меньше 365 (год истории склада)")
```
...
**Почему пол 365, а дефолт 730.** 730 — это ровно потолок `?days=` у
`/vending/stock-counts` (`STOCK_COUNTS_DAYS_MAX`, `vending.service.ts:144`):
при умолчании лист истории видит ВСЁ, что вообще может запросить. Пол 365
оставляет владельцу осознанное «хранить год», ниже — панель не примет.
Это МЯГЧЕ приёма П8b, где пол равен самому широкому потребителю (180 у
снимков): плата за разницу названа прямо в тексте `help` — «ниже 730 лист
«История склада» на окне 730 покажет обрезанную историю, вернуть её нечем».

**Чем платим, если ошиблись.** Повесив историю на `SNAPSHOT_RETENTION_DAYS`,
теряем полгода ручных инвентаризаций одним воскресеньем и без следа. Не
поставив пол — то же самое, но опиской в env. Взяв `counted_at` — строка с
`dt` внутри окна чтения может исчезнуть, и лист покажет дырку без объяснения.
...
const stockCountDays = Math.max(
  365, // пол R-H-8: ниже панель не примет; дефолт 730 = потолок ?days= у /vending/stock-counts
...
**`docs/DATA_SOURCES.md`**: абзац «сколько живёт история склада» с числом
дефолта, полом и прямой фразой про то, что ниже 730 лист «История склада» на
окне 730 покажет обрезанную историю.
...
* **Новая настройка** (`config-spec.ts`): `STOCK_COUNT_RETENTION_DAYS`,
  `fallback "730"`, `validate: atLeast(365, …)`, русский `help` (R-H-8).
...
* «пол 365 держится и против env: панель такое отобьёт, окружение — нет»;
...
6. Отложенная проверка ретенции: в ближайший понедельник посмотреть события
   ...
   2025-08-17 при пороге 730). Появление записи = сигнал, что порог кто-то
   опустил.
