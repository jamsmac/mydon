...
Шесть отклонений плана от этой спеки, каждое принято ДО начала кода
(зафиксировано в ledger `.superpowers/sdd/2026-08-26-sloy-p7-tasks/progress.md`
перед Task 1) и подтвердилось верным по факту реализации.

**«Подтверждено» — колонки, а не пятое значение `taskStatusEnum`.** Спека
не называла механизм явно; план выбрал `confirmed_at`/`confirmed_by` поверх
существующего `status='done'` (R-P7-6), потому что пятое значение enum молча
меняло бы смысл десяти существующих условий вида `ne(status,"done")` —
компилятор такую ошибку не ловит. Цена: `taskState()` в `@mydon/shared`
считает производное состояние отдельной функцией, а не читает поле напрямую.

**Мост «событие → задача» — крон Core поверх `EventsService.list`, а не
подписка.** Правила читаются опросом раз в сутки (06:15), а не событийной
подпиской: в Core уже есть ровно этот паттерн (`sync-stale.service.ts`), а
подписка потребовала бы новой инфраструктуры ради пяти источников с
дневным SLA, где секунды задержки не имеют значения.

**Автозадачи рождаются свободными, без исполнителя.** `VENDING_ROUTE_ORDER`
— порядок отображения маршрута, а не список закреплений «автомат → человек»;
назначать автозадачу конкретному сотруднику значило бы придумывать
привязку, которой в данных нет. Свободная задача видна всем в общем пуле.

**Тихие часы (R-P7-11) — только для двух НОВЫХ петель.** «Тебе поручили» и
«выполнена — подтвердите» подчиняются `внутриРабочихЧасов`; существующие
напоминания и возвраты (`REDO_NOTIFY_INTERVAL_MS`, `REMIND_INTERVAL_MS`) —
нет. Их поведение — приёмка прошлых срезов, и П7 её не трогает: расширение
тихих часов на весь бот — отдельное решение с отдельной ценой (сотрудник в
поле может не увидеть срочное ночью).

**Рунбуки — `docs/AGENTS_ACTIVATION.md`, не `docs/AGENTS.md`.** Спека и
`docs/AGENTS.md` описывали план README, которого в репозитории не существует
(проверено в рабочем дереве до начала кода); `AGENTS_ACTIVATION.md` — реальный
дом всех разделов «крон/петля X делает Y, тумблер Z», и Task 8 продолжает
этот файл, а не заводит второй источник истины.

**Ожидание первого утра — 0–3 автозадачи, не «задачи появятся».** Спека не
давала числа; план вывел верхнюю границу из 14-суточного замера ленты
событий (§3 описи) ДО деплоя, чтобы «ноль в первое утро» не читалось как
сбой моста — оно означает «парк молчал», а не «мост не работает», и
отличить одно от другого можно только вместе с шагом 8 (Task 8, Step 5).
