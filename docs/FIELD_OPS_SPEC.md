...
| `person.assignment_added` / `_removed` | закрепление за объектом | human/owner | personId |
...
```ts
export const staffScopeEnum = pgEnum("staff_scope", ["machine", "point"]);
...
/**
 * За что человек отвечает: конкретный автомат или кофе-точка. Периодами, как
 * coffee_machine_placement: «кто отвечал в мае» — рабочий вопрос при разборе
 * просрочки, и отвечать на него должен не нынешний ответственный.
 *
 * Ссылка полиморфная (как в attachment): machine → entity.id,
 * point → coffee_location.id. Две nullable-FK ради ссылочной целостности
 * читаются хуже, чем одна пара (scope, ref_id).
 */
export const staffAssignment = pgTable(
  "staff_assignment",
  {
    id: id(),
    personId: uuid("person_id").references(() => person.id, { onDelete: "cascade" }).notNull(),
    scope: staffScopeEnum("scope").notNull(),
    refId: uuid("ref_id").notNull(),
    /** false — подменный: видит объект, но основной ответственный другой. */
  ...
  (t) => [
    index("staff_assignment_person_idx").on(t.personId, t.scope),
    index("staff_assignment_ref_idx").on(t.scope, t.refId),
    // Один открытый «основной» на объект: двое ответственных = ноль ответственных.
    uniqueIndex("staff_assignment_open_primary_key").on(t.scope, t.refId)
      .where(sql`end_date is null and is_primary`),
    uniqueIndex("staff_assignment_open_key").on(t.personId, t.scope, t.refId)
...
```ts
  ...
  machinePart,
  maintenancePlan,
  // Доступ сотрудников: приглашения и закрепление за объектами.
  staffInvite,
  staffAssignment,
```
...
| 0043 | `0043_staff_roles` | `ALTER TABLE person ADD COLUMN roles text[] DEFAULT '{}' NOT NULL`; enum `staff_scope`; таблицы `staff_invite`, `staff_assignment` |
...
| `t:` | `t:<u>:<progress\|done\|open>` | задача (расширяем существующий регекс на `open`) | 47 |
...
**Уровень 1 — недавние (MRU), 5 штук.** Источник: `maintenance_log(performed_by_id, entity_id, performed_on desc)` + `collection(operator_id, machine_id, collected_at desc)`. Нужно добавить в Core `GET /maintenance/recent-objects?personId=&limit=5` и метод `CoreClient.recentObjects(personId)`. Реально закрывает большинство случаев: техник обслуживает свой куст точек, а не всю сеть.
...
```
Один объект у сотрудника (по `staff_assignment` или ровно одно совпадение) — шаг пропускается:
```
...
```
🗓 На ближайшие 14 дней по твоим объектам ничего не подходит.
Всё сделано, ничего не просрочено. 👌
```

Список фильтруется по объектам сотрудника: `staff_assignment` (открытые периоды) ∪ объекты его открытых задач. Показывать технику 47 автоматов сети — верный способ, чтобы он перестал открывать раздел.
...
### 6.4 Назначение исполнителя

Три тира, первый сработавший выигрывает:

```ts
export type AssigneeSource = "plan" | "object" | "defacto" | "none";

/**
 * Кому поручить работу.
 *   plan    — явно указан в maintenance_plan.assignee_id;
 *   object  — открытый primary в staff_assignment на этот объект;
 *   defacto — кто делал эту же работу на этом объекте за последние 30 дней.
 *
 * Тир «единственный подходящий по роли» намеренно ВЫКИНУТ: person.role —
 * свободный текст без фильтра по домену, и «поручил кому-то похожему» хуже,
 * чем честное «никому» — оно создаёт ложное чувство, что работа кем-то взята.
 * Тир «ответственный за зону» тоже выкинут: entity.attrs["зона"] в данных
 * не существует ни в одной записи.
 */
async resolveAssignees(plans: PlanRow[], now: Date): Promise<Map<string, Assignee>>
```

Разрешение — батчем: одна выборка `staff_assignment` по `inArray(refId, targetIds)`, одна `DISTINCT ON (kind, entity_id) ... ORDER BY performed_on DESC` по `maintenance_log`.

**Если ответственного нет** — три действия одновременно, и ни одно из них не «назначить наугад»:

1. Событие `maintenance.unassigned` → правило `urgency: "briefing"` → строка в утреннем брифинге владельца.
2. Задача создаётся с `ownerRef = null` и попадает владельцу.
3. При `overdue` — событие `maintenance.overdue` с `assigneeId: null`, и правило владельцу бьёт пушем: несделанный технический осмотр не перестаёт быть проблемой оттого, что некому его поручить.
...
**Утренний дайджест сотрудника, 07:00** (раньше владельческого брифинга 07:30 — владелец должен видеть, что люди уже получили). Строится из `myTasks` + `dueList`, группируется **по объекту**, а не по виду работ: техник ездит по точкам, а не по видам.
...
export async function runMaintenanceMonitor(
  ...
  now = new Date(),
): Promise<{ tasks: number; overdue: number; unassigned: number; errors: string[] }>
```
...
- `assigneeId === null` → событие `maintenance.unassigned`.
...
| `maintenance.unassigned` | `maintenance-monitor` | `{planId, kind, kindLabel, targetName, dueDate, daysLeft}` | агент |
...
},
{
  ...
  eventType: "maintenance.unassigned",
  urgency: "briefing",
  format: (c) =>
    `🙋 Некому поручить: ${str(c.payload.kindLabel)} — ${str(c.payload.targetName)}, ` +
    `срок ${str(c.payload.dueDate)}. Назначьте ответственного в карточке объекта.`,
},
{
...
**Блок «Графики» (верх).** Таблица планов: объект · вид работ · узел · периодичность · последний раз · следующий срок · статус · ответственный · «уведомлён» (иконка — есть ли задача с `source LIKE 'maint:<planId>:%'`). Цвета — те же четыре: 🔴 / 🟡 / 🟢 / ⚪. Счётчик в шапке: `Просрочено N · Скоро M · Норматив не задан K`.

Действия: «Новый график» (объект, вид, узел, периодичность, ответственный, `task_lead_days`), «Правка», «Выключить» (`is_active = false`, а не удаление — исключение для отдельного автомата выражается именно так), «Применить к списку объектов» (массовое создание планов по шаблону — заменяет модельные шаблоны).
...
### 7.4 `/team` и `/team/[id]` — роли, доступ, закрепления
...
- Карточка сотрудника: блок «Доступ» (статус привязки, «Выпустить приглашение» / «Отозвать доступ» / «Уволить»), блок «Закреплённые объекты» (`staff_assignment`, добавить/закрыть период), блок «Работы за 30 дней» (число записей `maintenance_log`, доля `outcome='done'`).
...
- `apps/core/src/tasks/tasks.service.ts` — `entityId?: string` в `CreateTaskInput`, проброс в `create()`; `ensureForDay` переписать на `insert … onConflictDoNothing({ target: task.source }).returning()`
- `apps/core/src/tasks/tasks.controller.ts` — `@IsOptional() @IsUUID() entityId?: string` в `CreateTaskDto` и в `EditTaskDto`, маппинг
...
- `apps/core/src/tasks/tasks.service.test.ts`: «повторный ensureForDay на тот же день не создаёт вторую задачу»; «entityId сохраняется и возвращается».
...
- `apps/core/src/maintenance/maintenance.service.ts` — `dueList()`, `resolveAssignees()`, `plans()`, `upsertPlan()`, `deactivatePlan()`; замыкание цикла в `closeMaintenanceLog` (`greatest()`, сдвиг якоря `due_on`, закрытие связанной задачи)
...
- Задача имеет `entityId`, `ownerRef` = ответственный, `due` = дата срока.
...
**Тесты:**
- `maintenance-monitor.test.ts` со stub-клиентом-накопителем вызовов (образец — `staff-register.test.ts:26-45`): «unknown пропускается»; «срок сегодня просрочкой не считается»; «просрочка 3 дня даёт событие, 4 дня — нет»; «план без ответственного даёт `maintenance.unassigned`»; «ошибка одного плана не роняет остальные».
...
**Критерии приёмки:**
- «🗓 Графики» показывает работы **только по объектам этого сотрудника**, сгруппированные по четырём статусам, ≤12 позиций в сообщении.
...
**Тесты:**
- `staff-digest.test.ts`: формат при 0 / 1 / 15 работах; группировка по объекту; ≤4096 символов на 30 работах.
- `schedules.test.ts`: парсер `sc:`; `unknown` рисуется ⚪, не 🟢.
...
**Изменить:**
- `packages/db/src/schema.ts` — `person.roles`, `staffScopeEnum`, `staffInvite`, `staffAssignment`, регистрация
...
Отсортированы по влиянию на архитектуру первой версии. Первые пять блокируют проектные решения — ответы нужны до PR 4.

1. **Кто закреплён за каждым объектом по умолчанию и сколько полевых сотрудников?**
   Без ответственного автосозданная задача ТО уходит владельцу, `delivered = true`, `markReminded` ставится — и задача гасится навсегда после одного пуша. Это блокер PR 7. Нужен список «объект → человек» хотя бы черновиком.

2. **Сколько автоматов и каких моделей — отдельно кофейные, снек, дринк?**
   От модели зависит состав узлов в `part_kind` и реалистичная периодичность. Список из 21 значения составлен по типовому парку; если в парке есть узлы, которых там нет, лучше добавить их до миграции, чем `ALTER TYPE` потом.

3. **Реальная периодичность работ, которую владелец готов подтвердить.**
...
4. **Ведутся ли серийные номера узлов и куда девается снятая деталь?**
...
5. **Есть ли действующая гарантия и сервисные контракты от поставщиков автоматов?**
...
6. **Разделение ролей: оператор, техник, инкассатор — совмещают или нет?**
...
7. **Технический осмотр — что владелец имел в виду?**
...
8. **Требуется ли журнал санитарной обработки установленной формы?**
...
9. **Нужна ли перерегистрация фискальной кассы при перестановке автомата на другую точку, и в какой срок?**
...
10. **Попадают ли товары дринк-автомата под обязательную цифровую маркировку?**
...
11. **Ручной ввод показаний счётчика: у каких автоматов он реально доступен?**
...
12. **Язык интерфейса бота: только русский или нужен узбекский?**
