...
| **Пополнение снек/дринк-автомата из бота (`fl:`)** | Писать некуда: `machine_slot` — зеркало OurVend (`ingestSlots` перетирает `quantity` по крону `0 */3 * * *`), `vending_stock` — центральный склад по имени товара. Нужна отдельная таблица `vending_refill` и отдельное решение владельца. **Готово (срез П4, 2026-08-25):** мастер «📦→🍫 Заполнил автомат» реализован под префиксом `rf:` (не `fl:` — свободный префикс на момент этой итерации оказался занят другим мастером), чек-лист по плану закупа, пишет `vending_refill`; отдельное решение владельца принято — заливка есть факт снимка (детектор `vending_refill_event`), запись оператора уточняет и остаётся единственным источником списания склада (R-P4-2). |
