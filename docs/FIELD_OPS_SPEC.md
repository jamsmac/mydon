...
| **Пополнение снек/дринк-автомата из бота (`fl:`)** | Писать некуда: `machine_slot` — зеркало OurVend (`ingestSlots` перетирает `quantity` по крону `0 */3 * * *`), `vending_stock` — центральный склад по имени товара. Нужна отдельная таблица `vending_refill` и отдельное решение владельца. **Готово (срез П4, 2026-08-25):** мастер «📦→🍫 Заполнил автомат» реализован под префиксом `rf:` (выбор реализации; `fl:` из этой строки на момент выкатки свободен и остаётся так же не занят — префикс не был причиной), чек-лист по плану закупа, пишет `vending_refill`; отдельное решение владельца принято — заливка есть факт снимка (детектор `vending_refill_event`), запись оператора уточняет и остаётся единственным источником списания склада (R-P4-2). |
...
**Поправка на текущий масштаб.** Полевых сотрудников двое, и оба делают всю работу — им обоим проставляется полный набор ролей. Шаги 2-3 (скрипт переноса) нужны только потому, что в `person` лежат не только полевые: там же контакты GLOBERENT и прочие карточки, и раздавать им права по свободному тексту `role` нельзя. Если сухой прогон покажет, что привязанных к Telegram карточек считаные единицы, скрипт из PR 10 можно выбросить и проставить роли руками в `/team` — это честнее, чем поддерживать одноразовый инструмент.

Матрица прав при двух сотрудниках почти ничего не режет. Она нужна не сейчас, а в момент появления третьего человека с урезанным доступом: тогда это одна строка в `STAFF_ROLES`, а не переделка меню и хендлеров бота.
...
- `packages/db/src/backfill-roles.ts` — **условно**: писать только если сухой прогон по `person` покажет заметное число привязанных карточек. При двух полевых сотрудниках роли проставляются руками в `/team`, и одноразовый скрипт не окупается (§3.3)
...
- Меню сотрудника без права `parts.replace` не содержит «🔧 Замена детали», а слово «заменил» получает вежливый отказ, а не запуск мастера. Проверяется тестом: у обоих действующих сотрудников полный набор ролей, и вручную этот случай в бою не воспроизвести.
- Пустые `roles` не запирают человека: «Мои задачи» работают (BASELINE).
- Бэкфилл без `--apply` ничего не пишет и печатает список неопознанных (если скрипт вообще делается).
...
| Сколько полевых сотрудников | **Двое: Владимир (Володя) и Рустам** | Матрица прав в V1 почти ничего не режет — оба делают всю работу и получают полный набор ролей. Это не повод от неё отказаться: она нужна, чтобы третий человек с урезанным доступом добавлялся одной строкой, а не переделкой бота. Пункт 4 раскатки (§3.3) сводится к двум карточкам. Рассылка дайджеста — два сообщения, лимиты Telegram здесь не проблема; ограничитель исходящих (PR 9) нужен для `sendReminders` и карточек задач, а не для дайджеста |
...
3. **Ведутся ли серийные номера узлов и куда девается снятая деталь?**
   ...
   > **Отвечено 04.09.2026:** вводится **свой инвентарный номер** на каждый узел (проставляется постепенно, по
   > напоминанию), серийник и фото шильдика — где есть; снятая деталь уходит на мойку/ремонт/склад и остаётся в
   > учёте. Проект — `docs/superpowers/specs/2026-09-04-vendhub-parts-inventory-design.md`.
...
| **Роли `driver` и `viewer`** | В поле сегодня два человека, и оба делают всю работу. Добавляются одной строкой в `STAFF_ROLES` и тремя в `ROLE_PERMISSIONS` | Когда появятся такие люди |
