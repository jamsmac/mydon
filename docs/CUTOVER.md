...
- `readyForCutover: false` → рано, не флипать. Смотреть `days` (последние 14
  суток, ТОЛЬКО ДЛЯ ПОКАЗА) — какой день красный или пропущен.
- `lastRed` — дата последнего НЕзелёного дня СТРОГО до начала текущей
  ...
  Ищется НЕ по 14-дневному списку `days` (тот — только витрина), а по
  ВСЕМУ прочитанному журналу события `ourvend.parity` — `parityScanLimit`
  строк (не меньше 60, не больше 400, растёт вместе с порогом серии):
  `lastRed: null` значит «красного дня не было ни разу за всё это окно»,
  а не только за последние две недели. Значение справочное, не приговор
  серии: бот печатает «последний красный день: 06.08» (формат дд.мм),
  панель — «последний красный день: 06.08.2026» (дд.мм.гггг); при
  `lastRed: null` — «красных дней не было». Слов «серия сорвана»/
  «обнулена»/«сброшена» нет нигде.
...
# ожидаем: value: "own", source: "db", effective: "own"
...
и при флипе. Альтернатива — первый пункт списка панель подписывает «— выкл
—» (вернуться к значению из env/умолчанию): запись с пустой строкой
УДАЛЯЕТ ключ из `system_config`, и действует фолбэк — env
`OURVEND_ACCOUNTING_SOURCE=stock` из `.env`/compose (см. «Что НЕ трогать» —
сам env-ключ на этом шаге не трогаем). Оба способа дают один и тот же
действующий источник — `stock`.
