...
`ourvend.parity` поля `день_ok`, `день_продаж_сверено`,
`день_остатков_сверено`, `день_расхождений` — именно их читает счётчик
серии (`parityStreak`). `GET /ourvend/health.parity` по-прежнему
показывает 7-дневную сверку — это витрина для человека, не источник
вердикта серии; числа витрины и числа дня не обязаны совпадать.
...
- `lastRed` — дата последнего НЕзелёного дня СТРОГО до начала текущей
  ...
  Значение справочное — «последний красный день: <дата>», не сигнал
  тревоги; `lastRed: null` значит «красных дней в просмотренном окне
  (14 суток) не было вовсе».
...
фолбэка `own` без `STOCK_DATABASE_URL` (панель показывает именно его, не
сырое `value`).
...
`{from:"own", to:"stock"}` появится тем же образом, что и при флипе.
Альтернатива — первый пункт списка «— по умолчанию (env) —»: он не пишет
значение в `system_config`, и действует фолбэк — env
...
В логе и в payload события `supply.sync` — серийники пропущенных
автоматов. Число строк ЗА ОДИН ПРОГОН — не всегда 34: окно чтения
снапшота берёт `fetched_at > now() - interval '3 days'`, и если снапшот
донора несёт несколько суток подряд (например, после простоя агента),
один прогон видит SKLAD 4S по каждым из них — от 34 (один день в окне)
до 102 (все три) строк за прогон. Устойчивый вклад в сутки остаётся 34.
...
- `GET /ourvend/parity/streak` продолжает расти. Пока `STOCK_DATABASE_URL`
  ...
  не идемпотентность своего снапшота самого с собой.
- Сторож застоя снапшота молчит: `GET /ourvend/health?runs=20` →
  ...
  `ourvend.snapshot_stale` уже пришло в Telegram, текст называет, какая
  таблица встала (`⛔ Учётный снапшот OurVend (продаж/остатков) не
  обновлялся N ч`); чинить агент `ourvend:accounting`, к шагу 3 не
  переходить.
...
5. **Паритет переходит в режим `retired`.** Без `STOCK_DATABASE_URL` свой
   ...
   ```bash
   curl -s http://127.0.0.1:3001/ourvend/health | jq '.parity.mode'
   # ожидаем: "retired"; note: «зеркала нет — сверять не с чем»
   ```
   Серия зелёных дней больше не считается, `ourvend.cutover_ready` не
   эмитится — оно и не нужно, катовер уже позади. Бот и панель
   («Здоровье сбора») вместо строки «N зелёных дн. подряд» говорят
   «сверка с зеркалом завершена».
