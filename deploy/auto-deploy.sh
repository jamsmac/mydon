...
# ЗАСТАВА ПУБЛИКАЦИИ ПАНЕЛИ — в РАБОЧЕМ пути (таймер), а не только в ручном
# deploy.sh. `PANEL_BIND` обязан быть localhost или Tailscale (100.64.0.0/10):
# опечатка `0.0.0.0` молча опубликовала бы панель без пароля в интернет,
# причём docker-proxy обходит ufw (ТЗ §6).
# СТОИТ В НАЧАЛЕ ПРОГОНА: застава читает только .env и ни от чего ниже не
# зависит. На шаге 4б (как раньше) отказ был дороже необходимого: каждый
# ретрай заново молол pg_dump+сборку+миграции, on_exit оставлял mydon-agents
# на паузе (fail-closed), а ротация «последние 10» за ~10 ретраев вытесняла
# единственный домиграционный дамп. Здесь отказ мгновенный и бесплатный:
# старый код, старая схема, агенты живы.
# РАЗБОР .env — ПО ПРАВИЛАМ COMPOSE, а не «grep '^PANEL_BIND='»: compose
# (env-file) принимает `export PANEL_BIND=…` и пробелы вокруг `=` — узкий
# grep эти строки не видел, panel_bind оставался пуст, и проверка молча
# пропускалась (fail-open; проверено на docker 28.3.3 / compose v2.39).
# Кавычки и CR compose срезает — срезаем и мы, иначе легитимный
# PANEL_BIND="100.x.y.z" блокировал бы каждый автодеплой (fail-closed).
panel_bind="$(grep -E '^[[:space:]]*(export[[:space:]]+)?PANEL_BIND[[:space:]]*=' "$APP_DIR/.env" 2>/dev/null \
...
if [ -n "$panel_bind" ]; then
  case "$panel_bind" in
    ...
    *)
      echo "PANEL_BIND='$panel_bind' не localhost и не Tailscale-адрес — деплой остановлен (панель ушла бы в интернет)" >&2
      exit 1
      ;;
  esac
fi
...
# 3а. Конфиг mydon-db (потолок логов и т.п.) иначе НЕ доехал бы никогда:
#     rollout ниже делает `up -d --no-deps` только для core/bot/cc/agents, а
#     `compose run` (миграции/сид) запущенные зависимости не пересоздаёт —
#     mydon-db молча оставался бы на безлимитном json-file при 38 ГБ диска.
#     Compose трогает контейнер только при реальном изменении его конфигурации,
#     и пересоздание безопасно: это rollback/recovery БД — прод-Core ходит в
#     managed DATABASE_URL (см. docker-compose.yml). Стоит ДО остановки
#     Agents: сбой здесь оставляет старый контур нетронутым, а миграции ниже
#     дождутся healthy через depends_on.
"${COMPOSE[@]}" up -d --no-deps mydon-db

# 3б. До migration гасим именно Agents. Старый Core/Bot/CC продолжают
