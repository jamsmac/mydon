...
# Сбой df или чужой формат вывода давали пустой USED: сравнения превращались в
# мусор, а в Core уходило usedPercent: 0 — «диск свободен» — с кодом возврата 0.
# Молчаливый сторож, докладывающий о здоровье, хуже отсутствующего.
if [[ ! "$USED" =~ ^[0-9]+$ ]]; then
    log "ОШИБКА: df не дал процент заполнения (получено: '${USED}')"
    exit 1
fi
