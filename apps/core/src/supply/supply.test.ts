...
describe("Снабжение: подготовка строк источника", () => {
  ...
  it("карантин остатков несёт dt и КАНОНИЧЕСКИЙ серийник раздельно — их читает уборка", () => {
    // `key` — склейка «серийник|сырой dt», по нему уборке день не сматчить:
    // без раздельных полей в формах values (дата без времени, канон серийника)
    // карантинный товар попадал бы под `NOT IN` и терял последнюю годную строку.
    const [q] = buildStockUpserts(
      [{ dt: "2026-07-28T00:00:00", machine_serial: "C2508160376", ourvend_name: "Вода", qty: "полно", fetched_at: new Date() }],
      new Map(),
    ).quarantined;
    assert.equal(q!.dt, "2026-07-28", "дата обрезана до суток, как у values");
    assert.equal(q!.machineSerial, "2508160376", "серийник канонический, как у values");
  });
});
...
interface МирСинка {
  ...
  /**
   * Содержимое зеркала `machine_stock` ДО прогона — для честной симуляции
   * уборки. Не задано — каждый DELETE «сносит» ровно одну строку (видно, что
   * счётчик берёт ДЛИНУ `returning`, а не число запросов).
   */
  зеркало?: { dt: string; machineSerial: string; product: string }[];
}
...
function стендСинка(м: МирСинка) {
  ...
  /**
   * Запросы уборки зеркала: текст условия и параметры. Рендерим настоящим
   * диалектом (тот же приём, что у окна сводки выше и у
   * `retention.service.test.ts`) — проверяем ТО, ЧТО УЙДЁТ В БАЗУ, а не
   * догадки о внутренностях drizzle.
   */
  const удаления: { sql: string; параметры: unknown[] }[] = [];
  const диалектУборки = new PgDialect();
  /** Живое «зеркало» machine_stock для симуляции: DELETE реально сокращает его. */
  ...
  const db = {
    ...
    delete: () => ({
      where: (условие: SQL) => ({
        returning: async () => {
          const q = диалектУборки.sqlToQuery(условие);
          удаления.push({ sql: q.sql, параметры: q.params });
          ...
          // параметров закреплён соседним тестом: день, автомат, приехавшие
          // товары (у `machine_stock` источника в ключе нет).
          const [dt, serial, ...товары] = q.params as string[];
          const жертвы = зеркало.filter(
            (r) => r.dt === dt && r.machineSerial === serial && !товары.includes(r.product),
          );
  ...
  return {
    ...
    события,
    удаления,
    зеркало: () => (зеркало === null ? [] : [...зеркало]),
    обращенийКРеестру: () => обращения.n,
    строкиЛога: () => [...строки],
  };
}
...
// ── Уборка исчезнувших остатков (зеркало machine_stock) ─────────────────────

describe("Зеркало остатков умеет СОКРАЩАТЬСЯ", () => {
  const снимок = (product: string, качества?: Partial<StockLevelRow>): StockLevelRow => ({
    dt: "2026-08-25",
    machine_serial: "2508160376",
    ourvend_name: product,
    qty: 6,
  ...
  it("по приехавшему дню-автомату уходит DELETE всего, чего в наборе нет", async () => {
    // Строка, пропавшая у источника за закрытый день (товар переименовали,
    // строку правили руками), оставалась в `machine_stock` навсегда — и гейт
    // паритета краснел «лишним товаром», которого в автомате нет.
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const { svc, удаления } = стендСинка({ снапшот: [снимок("TUC Sour cream"), снимок("Fanta 500ml")] });
      await svc.sync();
      assert.equal(удаления.length, 1, "один приехавший день-автомат — один запрос уборки");
      const q = удаления[0]!;
      assert.match(q.sql, /"machine_stock"\."product" not in \(/, "сносится всё, чего нет в приехавшем наборе");
      assert.deepEqual(
        q.параметры,
        ["2026-08-25", "2508160376", "TUC Sour cream", "Fanta 500ml"],
        "условие — ровно уникальный ключ таблицы: день, автомат и приехавшие товары",
      );
    });
  });
  ...
  it("ПУСТОЙ НАБОР НЕ УДАЛЯЕТ НИЧЕГО: ни строк, ни запросов уборки", async () => {
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const { svc, удаления } = стендСинка({ снапшот: [] });
      assert.deepEqual(await svc.sync(), { purchases: 0, stock: 0 });
      assert.equal(удаления.length, 0);
    });
  });

  it("автомат НЕ В СТРОЮ не приезжает — и его сутки уборка не трогает", async () => {
    // Строки склада отфильтрованы ДО записи (R-P8b-4), значит по этому
    // серийнику приехавшего набора нет вовсе. Стереть по нему день значило бы
    // обнулить историю живших там остатков из-за смены статуса карточки.
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const { svc, удаления } = стендСинка({
        снапшот: [снимок("Заглушка", { machine_serial: "2508160360", qty: 199 })],
        неВСтрою: ["2508160360"],
      });
      await svc.sync();
      assert.equal(удаления.length, 0);
  ...
  it("день, целиком ушедший в карантин, тоже ничего не стирает", async () => {
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const { svc, удаления } = стендСинка({ снапшот: [снимок("TUC Sour cream", { qty: "полно" })] });
      await svc.sync();
      assert.equal(удаления.length, 0);
    });
  });
  ...
  it("день ЧАСТИЧНО в карантине: последняя годная строка карантинного товара ЦЕЛА", async () => {
    // Годная строка дня открывает ключ уборки, но бракованный товар у
    // источника НЕ исчезал: его прежняя строка зеркала обязана выжить (иначе
    // карантин терял бы строку целиком — хуже нуля), а реально исчезнувший
    // товар того же дня — уйти.
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const день = { dt: "2026-08-25", machineSerial: "2508160376" };
      const { svc, события, зеркало } = стендСинка({
        снапшот: [снимок("TUC Sour cream"), снимок("Fanta 500ml", { qty: "полно" })],
        зеркало: [
          { ...день, product: "TUC Sour cream" },
          { ...день, product: "Fanta 500ml" },
          { ...день, product: "Исчезнувший у источника" },
        ],
      });
      await svc.sync();
      assert.deepEqual(
      ...
      const синк = события.find((e) => e.type === "supply.sync")!;
  ...
  it("каждый день-автомат убирается СВОИМ запросом, и убранное уходит в событие", async () => {
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const { svc, удаления, события } = стендСинка({
        снапшот: [
          снимок("TUC Sour cream"),
          снимок("TUC Sour cream", { dt: "2026-08-26" }),
          снимок("TUC Sour cream", { machine_serial: "72ac181f0000" }),
        ],
      });
      await svc.sync();
      ...
      assert.equal(
        ...
        3,
        "DELETE в зеркале остатков обязан оставлять след в журнале",
      );
    });
  });

  it("уходит РОВНО исчезнувшая строка, повторный прогон — 0 удалений", async () => {
    // Живое «зеркало» вместо счётчика: первый прогон обязан снести ровно ту
    // строку, что исчезла у источника, не тронув чужие сутки; второй — не
    // найти жертв вовсе (идемпотентность уборки).
    await сОкружением({ STOCK_DATABASE_URL: undefined }, async () => {
      const { svc, события, зеркало } = стендСинка({
        снапшот: [снимок("TUC Sour cream"), снимок("Fanta 500ml")],
        зеркало: [
          { dt: "2026-08-25", machineSerial: "2508160376", product: "TUC Sour cream" },
          { dt: "2026-08-25", machineSerial: "2508160376", product: "Fanta 500ml" },
          { dt: "2026-08-25", machineSerial: "2508160376", product: "Исчезнувший слот" },
          { dt: "2026-08-24", machineSerial: "2508160376", product: "Вчерашний день — не трогать" },
        ],
      });
      await svc.sync();
      await svc.sync();
      const прогоны = события
        .filter((e) => e.type === "supply.sync")
      ...
      assert.deepEqual(
        зеркало().map((r) => r.product).sort(),
        ["Fanta 500ml", "TUC Sour cream", "Вчерашний день — не трогать"],
        "снесён ровно исчезнувший слот приехавшего дня; чужие сутки целы",
      );
    });
  });
});
