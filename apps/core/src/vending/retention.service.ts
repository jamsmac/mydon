...
export class RetentionService implements OnModuleInit, OnApplicationShutdown {
   ...
   * `system.retention`, когда что-то реально удалено ИЛИ когда чистка не
   * доехала — оборвана ошибкой (`aborted`) или бюджетом времени (`capped`):
   * «удалено 0» — не новость (ни строки в журнале, ни лишнего события за 52
   * воскресенья в году), а «не смогли удалить» и «не успели удалить» —
  ...
  async sweep(now = new Date(), опции: SweepOptions = {}): Promise<RetentionResult[]> {
    ...
    for (const t of targets) {
      ...
      try {
        ...
        // «УДАЛЕНО 0» — НЕ НОВОСТЬ, «НЕ ДОЕХАЛИ» — НОВОСТЬ. Молчание при
        // пустой таблице бережёт журнал от 52 записей ни о чём в году, но
        // порог `deleted > 0` дважды съедал отказ, о котором в журнале не
        // оставалось ни строки. Сначала — обрыв на ПЕРВОЙ пачке (блокировка,
        // обрыв соединения): `aborted`, ноль удалённых — и только `warn` в
        // логе. Потом тем же образом молчал `capped` при нуле: цель, до
        // которой воскресный бюджет не доехал ВОВСЕ (весь ушёл на бэклог
        // slot_snapshot), выходила из цикла на первой же проверке часов — и
        // при затяжном бэклоге таблица неделями оставалась нечищенной без
        // следа в журнале, объявленном единственным свидетельством чистки.
        // «Не смогли» и «не успели» — новости равного веса, цена — не более
        // четырёх лишних строк в журнале, и только в capped-воскресенья.
        ...
        if (deleted > 0 || aborted || capped || includeEmpty) {
        ...
        if (!dryRun && (deleted > 0 || aborted || capped)) {
          try {
            await this.db.insert(event).values({
              source: "system",
              type: RETENTION_EVENT,
              occurredAt: now,
              // `capped` — В ЖУРНАЛ НАРАВНЕ С `aborted`: прогон, обрезанный
              // бюджетом времени, снёс лишь часть старых строк (или ни одной)
              // и доберёт остаток только в следующее воскресенье. Без этого
              // поля он неотличим от полной чистки, а событие объявлено
              // единственным свидетельством чистки — «удалено 5000» читалось
              // бы как «всё, что было», при десятках тысяч оставшихся.
              payload: { table: t.name, deleted, olderThanDays: t.olderThanDays, capped, aborted },
            });
          } catch (e: unknown) {
