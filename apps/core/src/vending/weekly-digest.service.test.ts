...
/**
 * Сторож на ОДНУ И ТУ ЖЕ ошибку, заведённую в этом файле трижды (N3 → D1 → R6):
 * новая константа вклинивается между докблоком и константой, которую он
 * описывает, и докблок становится сиротой — в подсказках IDE его не увидит
 * никто, а читатель припишет его чужому значению.
 *
 * Проверяется по ИСХОДНИКУ (наборы Core гоняются по `dist`, CommonJS: `__dirname`
 * смотрит в `apps/core/dist/vending`, исходник — на два уровня выше). Не «нет
 * двух докблоков подряд» — так фолс-позитивом стал бы модульный заголовок
 * файла, — а точечно: у каждого текста предупреждения СВОЙ докблок прямо над
 * ним, и он про него.
 */
describe("Сторож структуры: докблоки текстов письма не отрываются от своих констант (R6)", () => {
  const код = readFileSync(path.resolve(__dirname, "../../src/vending/weekly-digest.service.ts"), "utf8");

  const ПАРЫ: [string, RegExp][] = [
    ["const СПРОСИТЬ_БОТА =", /КУДА ОТПРАВЛЯТЬ ВЛАДЕЛЬЦА/],
    ["const ПАРИТЕТ_ВНЕ_ОКНА =", /окно счёта серии не достаёт/],
    ["const ЖУРНАЛ_НЕ_ДОСТАЁТ =", /НЕ ДОСТАЁТ до отчётной недели/],
    ["const ПРОГОНЫ_ОБРЕЗАНЫ =", /Потолок чтения журнала прогонов сработал/],
  ];

  for (const [объявление, про] of ПАРЫ) {
    it(`${объявление} — докблок прямо над ним и про него`, () => {
      const где = код.indexOf(объявление);
      assert.notEqual(где, -1, `объявление «${объявление}» пропало из файла`);
      const выше = код.slice(0, где).trimEnd();
      assert.ok(выше.endsWith("*/"), `над «${объявление}» нет докблока — он достался соседу`);
      const докблок = выше.slice(выше.lastIndexOf("/**"));
      assert.match(докблок, про, `над «${объявление}» стоит ЧУЖОЙ докблок: ${докблок.slice(0, 120)}…`);
    });
  }
});
