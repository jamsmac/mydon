...
export class NormFactService {
   ...
   * «полном».
   *
   * ГРАНИЦЫ ИНТЕРВАЛА (ревью 1.3): полуоткрытые, `(from, to]` — тот же приём,
   * что в `collections.service.ts::reconcile()` (`o.ts > p.prev AND o.ts <=
   * p.collected_at`). `enteredDate`/`returnedDate` — календарная дата без
   * времени: при обычной смене бункеров в один визит заливка нового периода
   * и возврат закрывающегося получают ОДИН И ТОТ ЖЕ день. Замкнутый интервал
   * («оба конца включены») отдавал бы чашки этого дня в оба периода разом —
   * норма задваивалась бы, разница занижалась. `to` (день возврата, конец
   * закрывающегося периода) забирает день себе; `from` (тот же день, но уже
   * начало следующего периода) — исключается.
   */
  private normaFor(
    ...
    let чашекБезНормы = 0;
    for (const r of rows) {
      if (r.date <= from || r.date > to) continue; // (from, to] — см. ГРАНИЦЫ ИНТЕРВАЛА выше
      чашек++;
      if (!r.productId) {
