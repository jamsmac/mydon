...
export interface ИнтервалСверки {
  ...
  изъято: number;
  разница: number;
  /** Доля расхождения от ожидаемого, % (изъято меньше ожидаемого — отрицательная). null, если ожидание было нулевым (делить не на что). */
  доля: number | null;
...
export class CollectionsService {
  ...
  async reconcile(from: string, to: string): Promise<РезультатСверки> {
    ...
    const intervals: ИнтервалСверки[] = сырыеИнтервалы.map((r) => {
      ...
      const изъято = Math.round(Number(r.amount));
      const ожидалось = Math.round(Number(r.expected));
      const разница = изъято - ожидалось;
      return {
        id: r.id,
        ...
        изъято,
        разница,
        // null при нулевом ожидании — делить не на что (та же логика, что у rows.доля).
        доля: ожидалось > 0 ? Math.round((разница / ожидалось) * 10000) / 100 : null,
