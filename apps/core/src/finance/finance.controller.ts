...
export class ImportBankStatementItemDto {
  ...
  /**
   * Ключ идемпотентности строки выписки. Граница — не круглое число, а
   * арифметика по составу ключа (`bank-statement.ts`):
   * счёт(~30) + номер документа(64, предел ниже) + дата(10) + дебет(~20) +
   * кредит(~20) + назначение(1000, предел ниже) + разделители(10) +
   * суффикс счётчика(~5) ≈ 1160.
   *
   * ПОЧЕМУ НЕ 128, КАК БЫЛО. На настоящей выписке ключ доходит до 339
   * символов (медиана 320) — потому что назначение платежа у банка длинное,
   * до 283 символов. Со старым пределом валидатор отбивал 2021 строку из
   * 2440, а глобальный пайп с `forbidNonWhitelisted` роняет ВЕСЬ запрос
   * целиком: импортировано было бы ноль. Найдено живой проверкой на проде —
   * ни тесты, ни ревью этого не увидели, потому что фикстуры короткие.
   *
   * Колонка `money_flow.ext_id` в БД — `text`, длиной не ограничена, так что
   * запас здесь ничего не стоит. Резать ключ нельзя: он ЗАПЕКАЕТСЯ В ДАННЫЕ,
   * и обрезка внесла бы риск коллизии в то самое место, которое от неё
   * защищали.
   */
