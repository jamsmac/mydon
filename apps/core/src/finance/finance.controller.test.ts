...
describe("ImportBankStatementDto через ValidationPipe (main.ts: whitelist + forbidNonWhitelisted)", () => {
  ...
  it("НАСТОЯЩАЯ длина назначения платежа (283 символа) проходит пайп — короткая фикстура этого не ловила", async () => {
    // Замер по живой выписке 22.08.2026: назначение до 283 символов, из-за
    // чего extId доходит до 339 (медиана 320). Со старым пределом 128
    // валидатор отбивал 2021 строку из 2440, а глобальный пайп роняет ВЕСЬ
    // запрос целиком — импортировано было бы ноль. Ни один тест этого не
    // видел, потому что все фикстуры были короткими; поймала живая проверка
    // на проде. Поэтому здесь назначение настоящей длины, не сокращённое.
    const длинноеНазначение =
      "00668п/о, оплата за товары, услуги за 23.04.2026 по сервису №68235 " +
      "оплата за услуги vendhub cог-но дог.№b/d 15157 от 2025-02-26 через click. " +
      "сумма 494900.00 сум, без ндс, идентификатор платежа 0002014167, филиал 01145";
    assert.ok(длинноеНазначение.length >= 200, "фикстура обязана быть длинной, иначе она не стережёт предел");

    const titleRow = ["Информация об операциях по счёту за период"];
    const accountRow = ["", "20208000900000000001", "ООО ТЕСТ КОМПАНИЯ", "", "", "", "", "", "", "", "999999999"];
    const dataRow = [
      "24.04.26",
      "29896000804942713020",
      "ООО ТЕСТ КОНТРАГЕНТ",
      "0002014167",
      "Платёжное поручение",
      "01145",
    ...
    const items = parseBankStatement({ columns: titleRow, rows: [titleRow, HEADER_NAMES, accountRow, dataRow] });
    assert.ok(items[0]!.extId.length > 128, `ключ обязан быть длиннее старого предела, иначе тест ничего не проверяет (сейчас ${items[0]!.extId.length})`);

    const result = await pipe.transform({ items }, { type: "body", metatype: ImportBankStatementDto, data: "" });
    assert.equal(result.items.length, 1, "строка с настоящей длиной назначения обязана пройти пайп");
    assert.equal(result.items[0].extId, items[0]!.extId);
  });
