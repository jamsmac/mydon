 ...
 * FK-полем). Действия владельца, агентов и импортов сюда не входят — вопрос
 * ленты именно «что сделали операторы и кладовщики».
 */
...
const gr = (n: number | null): string => (n === null ? "?" : `${n}`);
...
export class ActionsService {
  ...
  async actions(from: string, to: string, personId?: string): Promise<ActionRow[]> {
    ...
    const people = new Map(
      (await this.db.select({ id: person.id, name: person.name }).from(person)).map((p) => [p.id, p.name]),
    );
    const out: ActionRow[] = [];
    ...
    // Расходники: строка на (точка, день) переписывается upsert-ом — берём
    // updatedAt как момент последнего ввода.
    const consumables = await this.db
      .select({
    ...
    for (const r of consumables) {
      push(
        ...
        "consumable",
        personIdOf(r.by),
        `💧 Расходники: ${r.placeName} · вода ${gr(r.water)}, стаканы ${gr(r.cups)}, крышки ${gr(r.lids)}`,
      );
    }

    // Мойки.
    const washes = await this.db
      .select({
    ...
    // Обслуживание: чистка/осмотр/замена узла — personId лежит FK-полем.
    const logs = await this.db
      ...
      .where(
        and(gte(maintenanceLog.performedAt, lo), lt(maintenanceLog.performedAt, hi), isNotNull(maintenanceLog.personId)),
      );
    for (const r of logs) {
      push(
        r.at,
    ...
    const collections = await this.db
      ...
      .where(and(gte(collection.collectedAt, lo), lt(collection.collectedAt, hi), isNotNull(collection.operatorId)));
    for (const r of collections) {
      push(
        r.at,
        "collection",
        r.pid,
        `📥 Инкассация: ${r.objName}${r.status === "cancelled" ? " (отменена)" : ""}`,
      );
    }

    // Склад: приход и корректировка инвентаризации.
    ...
    // Задачи: закрытые (кто закрыл — исполнитель) и заведённые сотрудником
    // (заявки о поломке и любые постановки из бота).
    ...
    // Карточки, заведённые сотрудниками (ждут утверждения или уже утверждены).
