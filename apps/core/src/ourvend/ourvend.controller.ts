...
export class OurvendController {
  ...
  /**
   * Серия зелёных дней паритета и сигнал «можно переключать» (R-P8b-2).
   *
   * Отдельным роутом от `/ourvend/parity`, а не полем в нём: тот считает
   * СЕГОДНЯШНЮЮ сверку четырьмя сырыми агрегатами, этот — читает журнал
   * вчерашних вердиктов. Смешать их значило бы платить за весь SQL паритета
   * ради счётчика, который смотрит только в `event`.
   *
   * Троттл тот же, что у отчётов: витрины бота и панели зовут его вместе со
   * «здоровьем».
   */
  @Throttle({ burst: { limit: 12, ttl: 60_000 }, sustained: { limit: 12, ttl: 60_000 } })
  @Get("parity/streak")
