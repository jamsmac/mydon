 ...
 * `размещена` из калибровки тары (`fillNet`/`returnNet` не `null`), поэтому
 * `залито`/`возвращено` у него никогда не `null`, когда `размещена: true` —
...
export interface NormFactPeriodRow {
  ...
  полнота: "полный" | "позиция неоднозначна" | "тара не откалибрована" | "нет размещения" | "рецепт неизвестен" | "нормы нет";
  разница: number | null;
}
