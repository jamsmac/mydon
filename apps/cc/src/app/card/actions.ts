 ...
 * в момент сохранения.
 */
const MANAGED_ATTR_KEYS = [
  "меню",
  "раскладка",
  "состав",
  "история цен",
  "история цены покупки",
  // Контрагент: массивы, которых нет в форме паспорта. Через форму значения
  // возвращаются строками, и правка любого поля расплющила бы их — карточка
  // выпала бы из списка направления, а снимок закупок исчез бы целиком.
  "направления",
  "что поставляет",
  "roles",
  // Ингредиент: массивы без своего редактора в форме. «закупки сахара» на
  // карточке «Сахар» — история ТРЁХ чеков розничных закупок (дата, магазин,
  // цена за кг, номер чека), разобранная вручную из фото в Telegram; больше
  // её взять неоткуда. «альтернативы» на «Сухом молоке» — запасной поставщик
  // (Топпинг ALMAFOOD), которым закрывают отсутствие товара у основного.
  // Правка карточки формой БЕЗ этой защиты превращает массив в строку
  // «[object Object],…» — и то, и другое исчезло бы безвозвратно.
  "закупки сахара",
  "альтернативы",
];
