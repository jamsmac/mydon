...
import { ProvisionButton } from "./parts-provision";
...
export interface PartsFilter {
  ...
  q?: string | undefined;
  /**
   * Прочие параметры адреса едут насквозь: во вкладке рабочего места это
   * `tab=park:parts`, и без него клик по фильтру уводил бы с вкладки на
   * дашборд направления.
   */
...
/**
 * Реестр узлов. `base` — адрес, на который ведут ссылки фильтров: свой экран
 * `/parts` или вкладка «Парк» рабочего места. Фильтры обязаны возвращать туда,
 * откуда пришли, иначе клик по плитке «без фото» выкидывал бы человека из
 * рабочего места на отдельную страницу.
 */
export async function PartsView({ sp, base = "/parts" }: { sp: PartsFilter; base?: string }) {
  let parts: PartUnit[];
