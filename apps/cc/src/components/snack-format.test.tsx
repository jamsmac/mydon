 ...
 *
 * Часть снек-листов — асинхронные серверные компоненты, ходящие в Core
 * (`SalesView`, `SupplyViews`, `VendingPanel`, `StockHistoryView`,
 * `RefillEventsView`): отрендерить их в юните дороже, чем прочитать файл, а
 * утверждение всё равно про исходник — «снек-лист не заводит своего
 * форматтера чисел».
 *
 * Список ЗАШИТ КОНСТАНТОЙ, поэтому пополнять его обязан тот, кто заводит
 * новый лист: докблок `money()` обещает, что регрессию ловит этот сторож, и
 * два листа этой же ветки («История склада», «Журнал заливок») в него не
 * попали — механизм на них не распространялся, а обещание распространялось.
 *
