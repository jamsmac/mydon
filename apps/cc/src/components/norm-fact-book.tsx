 ...
 * «нет заливки» и «нет возврата». `NormFactService` строит `размещена` из
 * калибровки тары (`fillNet`/`returnNet` не `null` одновременно), поэтому
...
const COVERAGE_LABEL: Record<NormFactPeriodRow["полнота"], string> = {
  "полный": "полный",
  "нет размещения": "тара набора не откалибрована — нетто посчитать нельзя (к размещению автомата не относится)",
