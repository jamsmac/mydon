 ...
 * закупками по счетам на 95%, задача 1), факт занижен в 1,5–1,7 раза, потому
 * что треть заливок физически не записана в журнал (`bunker-period.ts`) —
 * поэтому ни у одной причины здесь нет цвета тревоги (правило 1 брифа этой
 * задачи): 511 из 885 периодов сегодня полные, остальные 374 — дыры журнала,
 * а не воровство сырья.
 */
const COVERAGE_LABEL: Record<NormFactPeriodRow["полнота"], string> = {
  "полный": "полный",
  "нет размещения": "нет размещения — бункер не привязан к автомату в этот период",
  "нет заливки": "нет заливки — в журнале нет записи заливки за период",
  "нет возврата": "нет возврата — в журнале нет записи возврата за период",
