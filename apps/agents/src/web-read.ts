...
export function buildWebProposal(results: WebReadResult[]): Proposal | null {
  ...
  return {
    ...
    // Дедуп — по СОСТАВУ и ДОСТУПНОСТИ источников (url + ok/не-ok), отсортировано
    // по url. Волатильные chars/excerpt в сигнатуру НЕ идут: динамические
    // страницы (счётчики просмотров, метки времени) меняют объём и выжимку
    // каждый фетч — сигнатура от facts целиком не совпадала бы никогда, и навык
    // слал бы дубль каждый прогон. url+ok меняется, когда источник добавлен/убран
    // или лёг/поднялся, — то есть при СОДЕРЖАТЕЛЬНОМ изменении разведки, и не
    // «плывёт» на неизменном наборе. Владельцу при этом показываются полные facts
    // (с chars/excerpt) — урезан только ключ дедупа.
    signatureFacts: {
      sources: results
        .map((r) => ({ url: r.url, ok: isOk(r) }))
