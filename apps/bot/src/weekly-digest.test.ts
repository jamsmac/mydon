...
describe("Блок здоровья: сначала неделя, потом «сейчас» (R-H-9)", () => {
  ...
  it("неделя раньше начала журнала прогонов названа так, а не «сбор не запускался»", () => {
    ...
    const текст = formatWeeklyDigest(доЖурнала, []).parts.join("\n");
  ...
  it("про начало журнала владелец читает ОДИН раз: хвост письма ту же фразу не повторяет", () => {
    // Ядро шлёт ту же фразу предупреждением `health_unavailable`, а блок
    // здоровья печатает её НА МЕСТЕ лжи «сбор не запускался». Две одинаковые
    // строки в одном письме — то же, за что убрана пометка про обрезку.
    const доЖурнала = {
      ...ПУСТАЯ_НЕДЕЛЯ,
      from: "2026-07-27",
      to: "2026-08-02",
      weekHealth: { ...ПУСТАЯ_НЕДЕЛЯ.weekHealth, week: "2026-31", journalSince: "2026-08-06" },
      warnings: [
        { code: "health_unavailable", message: "Журнал прогонов начинается с 06.08.2026 — за эту неделю данных нет." },
    ...
    // Соседнее предупреждение того же КОДА при этом доезжает: гасим текст, не код.
