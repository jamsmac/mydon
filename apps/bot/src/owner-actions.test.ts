...
describe("Итоги для владельца: отчёт по людям", () => {
  it("группирует по людям, деятельные сверху, время по Ташкенту", () => {
    const rows = [
      row({ ts: "2026-08-18T05:30:00.000Z", personId: "22222222-2222-4222-8222-222222222222", personName: "Рустам", label: "📦 Приход: Зёрна +10 кг" }),
  ...
  it("сводка для брифинга: имена с числом действий, подсказка «итоги вчера»", () => {
    const line = summarizeActions([row(), row(), row({ personId: "22222222-2222-4222-8222-222222222222", personName: "Рустам" })]);
