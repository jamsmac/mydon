...
async function startMenuItem(
  ...
  switch (item.id) {
    ...
    case "coll": {
      // ВЕСЬ парк, включая склад и ремонт. Статус отвечает на вопрос «работает
      // ли аппарат», а не «пуста ли касса»: деньги лежат в купюроприёмнике до
      // физического вскрытия, а вскрывают его как раз тогда, когда аппарат
      // снимают с точки — и статус к этому моменту уже переключён. Бот —
      // единственный вход для записи сбора (панель умеет только принять или
      // отменить уже созданный), поэтому спрятать аппарат здесь значит оставить
      // наличные вне учёта.
      const machines = await deps.core.machines("vendhub");
      if (machines.length === 0) {
        return { reply: { text: "Автоматов в реестре пока нет — скажи владельцу." } };
      }
      // Клавиатура показывает 30 строк. Автоматов в парке 31, и один молча
      // выпадал из кассового списка — про обрезку говорим вслух, как это уже
      // делает общий пикер объекта.
      const хвост =
        machines.length > 30 ? `\n\nПоказал 30 из ${machines.length} — остальные скажи владельцу.` : "";
      return {
        reply: {
          text: `С какого автомата собраны деньги? Время зафиксируется в момент нажатия.${хвост}`,
