...
export const STAFF_MENU: readonly MenuItem[] = [
  ...
  //
  // Значок «🍫», а не «📦»: тот уже занят «Приходом», а оператор ищет пункт по
  // эмодзи — один символ на два пункта провоцирует промах (то же правило, что
  // у «🧮 Инвентаризации»). Подпись меняется безболезненно: пока пункт стоял
  // ready:false, кнопка не попадала ни на одну розданную клавиатуру.
