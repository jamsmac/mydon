...
export function строкаСнапшота(h: OurvendHealth): string | null {
  ...
  const давность = h.salesLagH == null ? "снимков нет" : `${RU(h.salesLagH)} ч`;
  return `⛔ учётный снапшот OurVend не обновляется ${давность} — продажи и остатки стоят`;
}
...
export function строкаСерии(h: OurvendHealth): string {
  ...
  return (
    `паритет: ${RU(h.parityStreak)} зелёных дн. подряд из ${RU(h.cutoverThreshold)}` +
