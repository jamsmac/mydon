...
export function состояниеСбора(h: OurvendHealth): string {
  ...
  // успешным сто лет назад. Строка застоя в обоих отчётах стоит ВЫШЕ этой
  // (`строкаЗастоя`: в «сверке» — первой строкой, в недельном письме — сразу
  // за числами недели, R-H-9) — вторая зелёная галка под ней читалась бы как
  // «на самом деле всё хорошо». Номер строки здесь не называем намеренно: он
  // уже разъезжался с текстом, а «выше» остаётся правдой при любой вёрстке.
  //
  // Гейт стоит ВЫШЕ ветки «отказы в окне» (N2, ре-ревью): та ветка тоже
  // печатает «✅» и раньше проверялась ДО застоя, так что «✅ Сейчас
  // собирается, но 2 отказа…» вставала рядом с «⛔ сбор стоит N ч» — ровно
  // авария 24.08 (12 отказов → один успех в 16:00 → крон замолчал), только с
  // другим числом отказов в тексте.
...
/**
 * «⛔ учётный снапшот не обновляется» — застой ГОТОВЫМ вердиктом Core
 * (`h.snapshotStale`, R-P8b-5), не сравнением лага с порогом.
 *
 * В режиме `stock` `snapshotStale` всегда `false` — снапшот там теневой,
 * его застой ничего не останавливает, и строка не появляется вовсе, а не
 * гасится дополнительным условием здесь: гасить нечего, Core уже решил.
 * `salesLagH` едет ТОЛЬКО показом давности — сравнивать его с порогом здесь
 * нельзя (то же правило, что у самого `snapshotStale` в `vending-reports.ts`).
 *
 * Стоит СРАЗУ ЗА `строкаЗастоя`: обе строки отвечают на один вопрос «почему
 * данные не едут», и разнесённые по разным концам письма выглядели бы как
 * две несвязанные аварии.
 */
export function строкаСнапшота(h: OurvendHealth): string | null {
  if (!h.snapshotStale) return null;
  // Давность НАЗВАНА половиной, а не приклеена к вердикту. Застой считается по
  // ОБЕИМ таблицам снапшота (R-FW-P2), а в ответе едет возраст только продажной
  // (`ourvend_sale_snapshot`): встали остатки при свежих продажах — и «не
  // обновляется 0,3 ч» читалось бы как опечатка отчёта. Какая именно половина
  // встала, называет событие сторожа; здесь честно печатается то число, которое
  // есть, с подписью, чьё оно.
  const давность = h.salesLagH == null ? "снимков продаж нет" : `снимок продаж — ${RU(h.salesLagH)} ч`;
  return `⛔ учётный снапшот OurVend не обновляется — продажи и остатки стоят (${давность})`;
}

/**
 * «паритет: N зелёных дн. подряд из M» — серия зелёных дней паритета к порогу
 * катовера (R-P8b-2), а не голое число: владельца интересует «сколько ещё
 * ждать», и без порога рядом «3» ничего не отвечает.
 *
 * Порог берётся из `h.cutoverThreshold` — своей семёрки здесь нет, иначе
 * строка разошлась бы с базой в тот же день, когда владелец подвинет
 * `CUTOVER_GREEN_DAYS` в панели «Система».
 *
 * `parityStreak === 0` печатается словами «серии нет», а не «0 зелёных»: ноль
 * в этой строке читается как «сегодня не сошлось», а на деле это может быть
 * «сверок ещё не было ни одной» — разные починки, разный текст.
 */
export function строкаСерии(h: OurvendHealth): string {
  // Зеркало погашено (шаг 3 рунбука) — серия не считается вовсе: гейт своё
  // отработал, катовер позади. «Серии нет» здесь читалось бы как «сегодня не
  // сошлось», а зазывать переключить учёт, который УЖЕ свой, — тем более
  // (R-FW-P3).
  if (h.parity.mode === "retired") {
    return "паритет: серия больше не считается — зеркало погашено, учёт свой";
  }
  if (h.parityStreak <= 0) return "паритет: серии нет";
  const готово = h.parityStreak >= h.cutoverThreshold;
  // Прилагательное склоняется по числу: первый день КАЖДОЙ серии — самый
  // частый момент, когда владелец эту строку читает, и «1 зелёных дн.» режет
  // глаз ровно в том тексте, который отвечает «сколько ещё ждать». «дн.»
  // остаётся сокращением: у него формы одни на все числа.
  return (
    `паритет: ${RU(h.parityStreak)} ${сущ(h.parityStreak, "зелёный", "зелёных", "зелёных")} дн. подряд ` +
    `из ${RU(h.cutoverThreshold)}` +
    (готово ? " — ✅ можно переключать учёт (Система → OURVEND_ACCOUNTING_SOURCE = own)" : "")
  );
}

/**
 * «последний красный день: 25.08» — ФАКТ, а не приговор серии (P4, R-G-4).
 *
 * `h.parityLastRed` едет полем `/ourvend/health`, а не отдельным роутом
 * `/ourvend/parity/streak`: Core считает его в том же `Promise.all`, что и
 * серию, и до среза «Гигиена» выбрасывал наружу. `lastRed` ищется по ВСЕМУ
 * прочитанному журналу, а показывается окно в две недели, поэтому дата почти
 * всегда лежит ВНЕ окна: прод-красный 25.08.2026 продержится в поле до конца
 * октября, хотя серия пойдёт с 26-го. Прочитать его как «серия сорвана»
 * значило бы держать гейт закрытым днём, который серия давно перешагнула.
 * Витрина говорит ровно одно: когда рвалось в последний раз.
 *
 * `null` — красных не было ВООБЩЕ, и это не «сегодня красный»: молчать здесь
 * значило бы оставить владельца гадать, чист журнал или строку забыли.
 */
export function строкаКрасногоДня(h: OurvendHealth): string {
  return h.parityLastRed === null ? "красных дней не было" : `последний красный день: ${день(h.parityLastRed)}`;
}
...
export function паритетСтрока(p: OurvendHealth["parity"]): string {
  // Зеркала нет вовсе (шаг 3 рунбука): половины паритета печатать НЕЛЬЗЯ —
  // «продажи: сверять нечего · остатки: снимков за период нет» на погашенном
  // зеркале читается как авария сбора, хотя это штатный конец катовера
  // (R-FW-P3).
  if (p.mode === "retired") {
    return "Паритет: сверка с зеркалом завершена — сравнивать больше не с чем";
  }
...
export function formatOurvendHealth(h: OurvendHealth): string[] {
  ...
  // Застой учётного снапшота — тот же вопрос «данные не едут», что и застой
  // сбора выше: строка стоит сразу за ней (R-P8b-5).
  const снапшот = строкаСнапшота(h);
  if (снапшот) lines.push(снапшот);
  ...
  // Серия — вывод из паритета, стоит СЛЕДУЮЩЕЙ строкой сразу после него,
  // а не в отрыве от него (R-P8b-2).
  lines.push(свежестьСтрока(h), паритетСтрока(h.parity), строкаСерии(h));
  // Дата последнего красного дня — под серией, которую она объясняет (P4).
  // Поле здоровья, а не второй запрос (R-G-4): второго роута тут больше нет,
  // ломаться нечему. В режиме `retired` серии нет вовсе (см. `строкаСерии`) —
  // старая дата там только сбивала бы.
  if (h.parity.mode !== "retired") lines.push(строкаКрасногоДня(h));
