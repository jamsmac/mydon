...
/**
 * «последний красный день: 25.08» — ФАКТ, а не приговор серии (P4).
 *
 * `lastRed` ищется по ВСЕМУ прочитанному журналу, а показывается окно в две
 * недели (`PARITY_STREAK_WINDOW`), поэтому дата почти всегда лежит ВНЕ окна:
 * прод-красный 25.08.2026 продержится в поле до конца октября, хотя серия
 * пойдёт с 26-го. Прочитать его как «серия сорвана» значило бы держать гейт
 * закрытым днём, который серия давно перешагнула. Витрина говорит ровно одно:
 * когда рвалось в последний раз.
 *
 ...
 */
export function строкаКрасногоДня(s: ParityStreak): string {
  return s.lastRed === null ? "красных дней не было" : `последний красный день: ${день(s.lastRed)}`;
}
 ...
 */
export function formatOurvendHealth(h: OurvendHealth, streak: ParityStreak | null = null): string[] {
  ...
  // Серия приезжает ОТДЕЛЬНЫМ роутом (`/ourvend/parity/streak`), и его отказ
  // отчёт не отменяет: строки просто нет. В режиме `retired` серии нет вовсе
  // (см. `строкаСерии`) — старая дата там только сбивала бы.
  if (streak && h.parity.mode !== "retired") lines.push(строкаКрасногоДня(streak));
