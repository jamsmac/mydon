...
function deps(over: Record<string, unknown> = {}) {
  ...
  const core = {
    coffeeLocations: async () => [{ id: LOC, name: "Olma office", isActive: true, operational: true }],
...
describe("Повторная проверка: дефекты, найденные во второй волне аудита", () => {
  ...
  it("D6: снек-заливка с меню точки начинается сразу, кофейное записанное цело", async () => {
    // Пункт стоял ready:false, и проверка требовала «пока не готово». Мастер
    // появился (П4), и работает общее правило кнопок меню: меню точки — не
    // «недописанный мастер» (всё записано), поэтому переход в другое дело идёт
    // без ложной тревоги, а сделанное по кофе остаётся в Core.
    const { d, calls } = deps({
      recentObjects: async () => [{ id: "55555555-5555-4555-8555-555555555555", name: "Olma" }],
    });
    await toVisitMenu(d);
    const res = await TX(d, "🍫 Заполнил автомат");
    assert.match(res.text, /Заполнил автомат/i, "мастер начался");
    assert.doesNotMatch(res.text, /пока не готово/i);
    assert.equal(calls.length, 1, "заливка бункера записана и никуда не делась");
  });
