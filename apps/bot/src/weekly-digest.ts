 ...
 *
 * Отдельной функцией, а не веткой внутри блока: у строки три состояния
 * (пусто / посчитано / посчитано с оговоркой), и в списке строк они читались
 * бы тремя вложенными тернарниками.
 */
function прогоныНедели(w: WeeklyHealth): string {
  ...
  const пометки = [
    ...(w.partialWeek ? ["неделя ещё идёт"] : []),
    ...(w.capped ? ["показаны не все прогоны недели"] : []),
  ];
