...
function прогоныНедели(w: WeeklyHealth, конецНедели: string): string {
  ...
  // отправить владельца искать поломку, которой не было. Поле читается через
  // `typeof`, а не по `!== null`: у старого Core его в ответе нет вовсе.
  const начало = w.journalSince;
  if (w.runs === 0 && typeof начало === "string" && конецНедели < начало) {
