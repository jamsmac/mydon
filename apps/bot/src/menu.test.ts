...
describe("Пункт без мастера не показывается и не ловится словом", () => {
  it("«Заполнил автомат» скрыт: мастера нет, а кнопка стирала бы начатое", () => {
    ...
    // Подпись УЗНАЁТСЯ: кнопка живёт на розданных клавиатурах, и не узнать её
    // значит пустить текст комментарием к задаче. Готовность проверяет
    // вызывающий и отвечает «пока не готово», не трогая начатое.
    assert.equal(matchMenuLabel("📦 Заполнил автомат")?.ready, false, "подпись узнаётся как неготовый пункт");
  });
