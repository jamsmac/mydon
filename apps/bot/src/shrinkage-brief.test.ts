...
describe("Усушка: форматирование", () => {
  it("по автомату: период, посчитанные дни, позиции и итог", () => {
    ...
    // «не в счёт из-за заливки», а не «с заливкой»: daysSkipped — дни,
    // ИСКЛЮЧЁННЫЕ из расчёта, и та же формулировка стоит в панели (UX#3).
    assert.match(первое, /📉 Olma за 14 дн \(дней посчитано 9, не в счёт из-за заливки 5\)/);
  ...
  it("ни одного посчитанного дня — «не считали», а не «недостач нет» (R-FW-7)", () => {
    // Все сутки окна оказались заливочными: товар не считался НИ РАЗУ. Сказать
    // «недостач нет» значит подтвердить расчёт, которого не было, — и ровно
    // так обходится контроль усушки шумящим источником.
    const непосчитанный = машина({
    ...
    const [первое] = formatShrinkage(отчёт({ machines: [непосчитанный] }));
    assert.match(первое, /не считали — все 14 дн\. периода были заливкой\/пропущены/);
    assert.doesNotMatch(первое, /недостач нет/);
    assert.doesNotMatch(первое, /без потерь/);
  });

  it("ни одного посчитанного дня и ни одной заливки — тоже «не считали»", () => {
    // Тихий автомат без позиций и без заливок уходил в «без потерь» ДО
    // проверки посчитанных дней — и пустой расчёт выглядел как чистый.
    const тихий = машина({
      summary: { items: [], lossValue: 0, daysCounted: 0, daysSkipped: 14, threshold: 30_000 },
      refillDays: [],
    });
    const [первое] = formatShrinkage(отчёт({ machines: [тихий] }));
    assert.match(первое, /не считали/);
    assert.doesNotMatch(первое, /без потерь/);
  });

  it("предупреждение «не по чему считать» доходит до владельца (R-FW-7)", () => {
    const parts = formatShrinkage(
      отчёт({
        warnings: [{ code: "no_counted_days", message: "Olma: не считали — все 14 дн. были заливкой" }],
      }),
    );
    assert.match(parts[parts.length - 1], /не считали — все 14 дн\. были заливкой/);
  });

  it("числа без неразрывного пробела — их копируют и ищут (UX#4)", () => {
    // toLocaleString("ru-RU") разделяет разряды U+00A0: скопированная из чата
    // сумма не находится поиском и не сходится при сравнении.
    const parts = formatShrinkage(отчёт());
    for (const p of parts) assert.ok(!p.includes("\u00A0"), "неразрывный пробел в отчёте");
    assert.match(parts[0], /порог 30 000 сум/);
    assert.match(parts[0], /≈ 99 000 сум/);
  });
