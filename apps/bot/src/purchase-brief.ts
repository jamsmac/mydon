...
export function formatPurchaseBrief(p: VendingPurchase): string {
  ...
  if (p.items.length > 0) {
    ...
    // «Переплата» читалась как «нас обсчитали». Это купленный впрок товар: он
    // не пропал, а уехал на склад — и владелец должен видеть, за что платит
    // сверх нехватки (UX#8).
    if (p.overpay > 0) lines.push(`Куплено сверх нехватки: ${RU(p.overpay)} сум (округление до блока + фикс-количества)`);
...
export function parsePriceCommand(text: string): PriceCommand | null {
  const разбор = разборЦены(text);
  return разбор === "non_positive" ? null : разбор;
}

/**
 * Цена, которую владелец назвал сам, но которой не бывает: «-15 000», «0».
 *
 * Отличать её от «фраза не разобралась» обязательно. Раньше знак минус
 * попадал в класс разделителей `[\s:—=-]` и молча съедался: «цена TUC -15000»
 * записывалась как 15 000 — ввод владельца переосмысливался на пути к записи.
 * Убрать знак из разделителей мало: «цена TUC 0» тоже не цена, а на подсказку
 * формата владелец ответит той же командой, потому что формат-то верный.
 */
export function isNonPositivePrice(text: string): boolean {
  return разборЦены(text) === "non_positive";
}

/** Ответ на цену, которой не бывает: причина словами, а не пересказ формата. */
export const NON_POSITIVE_PRICE_HINT =
  "Цена должна быть больше нуля — «0» и минус я не приму. " +
  "Если хотел уменьшить цену, назови новое значение: «цена TUC 12000».";

/**
 * Общий разбор для обеих цен (закупочной и эталона витрины).
 *
 * `"non_positive"` — число названо, но оно ≤ 0; `null` — фраза не разобралась
 * вовсе. Дальше по этой развилке расходятся ответы владельцу.
 */
function разборЦены(text: string): PriceCommand | "non_positive" | null {
  ...
  //
  // Разделитель захватывается ОТДЕЛЬНОЙ группой: дефис в нём остался (он
  // разделяет «TUC-12000» и живёт внутри имён вроде «Coca-Cola»), но знак
  // минус перед числом теперь виден — это `\s[-−]` на конце разделителя.
  const m =
    /^(.+?)([\s:—=-]*[\s:—=−-])(\d+(?:[\s\u00a0\u202f]\d{3})*)\s*([кk])?\s*(?:сум\.?|sum|uzs)?\s*[.!]?\s*$/i.exec(
      t,
    );
  if (!m) return null;
  if (/[\s:—=]\s*[-−]$/.test(m[2])) return "non_positive";
  const product = m[1].trim().replace(/[,;:—-]+$/, "").trim();
  if (!product) return null;
  const digits = m[3].replace(/[\s\u00a0\u202f]+/g, "");
  let price = Number(digits);
  if (m[4]) price *= 1000;
  if (!Number.isInteger(price) || price > 10_000_000) return null;
  // Ноль — не «не разобрал»: число названо, и владельцу надо сказать именно
  // про ноль, а не показать формат, которому его команда и так соответствует.
  // Потолок выше — защита от строки штрихкода, принятой за цену: дороже
  // 10 млн сум за единицу в этом бизнесе не бывает.
