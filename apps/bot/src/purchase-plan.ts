...
/**
 * Число разрядами — ЕДИНСТВЕННЫЙ формат денег и штук во всех отчётах бота
 * (план закупа, усушка, аналитика снека, недельная сводка).
 *
 * Две правки против того, что даёт `ru-RU`:
 * 1. Неразрывный пробел (U+00A0) меняем на обычный: сумму из отчёта копируют
 *    и ищут, а с U+00A0 она не находится и не сходится при сравнении (та же
 *    правка, что у formatAmount в правилах Core).
 * 2. Минус — типографский «−», а не ASCII-дефис: рядом в тех же строках стоят
 *    «−9 шт» и «(−20 %)», написанные руками, и одна отрицательная сумма
 *    дефисом выдаёт, что числа собирали в двух разных местах.
 *
 * Экспортируется, потому что копий этого формата в боте было три и они уже
 * разъезжались (U+00A0 чинили только в одной).
 */
export const RU = (n: number): string =>
  Math.round(n)
